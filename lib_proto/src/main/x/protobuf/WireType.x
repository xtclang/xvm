 ...
 *
 * Each field in a protobuf message is encoded with a tag that includes a wire type, which tells the
 * parser how to determine the size of the following value.
 *
 * The tag is encoded as a varint: `(fieldNumber << 3) | wireType`.
 */
enum WireType(Byte id) {
    /**
     * Variable-length integer encoding. Used for: int32, int64, uint32, uint64, sint32, sint64,
     * bool, enum.
     */
    VARINT(0),

    /**
     * Fixed 8-byte encoding (little-endian). Used for: fixed64, sfixed64, double.
     */
    I64(1),

    /**
     * Length-delimited encoding (varint length prefix followed by payload). Used for: string,
    ...
    /**
     * Fixed 4-byte encoding (little-endian). Used for: fixed32, sfixed32, float.
     */
    I32(5);
    ...
    static Int makeTag(Int fieldNumber, WireType wireType) =
        (fieldNumber << TagTypeBits) | wireType.id;
     ...
     */
    static Int getFieldNumber(Int tag) = tag >>> TagTypeBits;
     ...
     * @return the WireType
     */
    static WireType getWireType(Int tag) = WireType.values[tag & TagTypeMask];
}
