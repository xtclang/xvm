...
class XdkLanguageServerTest {
    ...
    fun `compiler mode advertises only its implemented features`() {
        Session(XdkAdapter()).use { session ->
            ...
            assertThat(capabilities.documentFormattingProvider.left).isTrue()
            assertThat(capabilities.documentLinkProvider).isNotNull()
            assertThat(capabilities.codeLensProvider).isNotNull()
            assertThat(capabilities.linkedEditingRangeProvider.left).isTrue()
