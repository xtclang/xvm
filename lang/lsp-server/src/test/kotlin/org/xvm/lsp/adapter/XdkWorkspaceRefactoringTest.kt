...
class XdkWorkspaceRefactoringTest {
    ...
    fun `inline types static functions and constants rename through unopened consumers`(name: String) {
        ...
        val consumer = source("Consumer", "module Consumer { package lib import Library; lib.Box make()=new lib.Box(); Int run()=lib.answer()+lib.number; }")
    ...
    fun `import cleanup preserves used aliases and comments and returns only compiled proofs`() {
        val text = "module App {\r\n" +
            ...
            "    import ecstasy.collections.ArrayList;\r\n" +
            "    // Keep this comment 😀\r\n" +
        ...
        XdkAdapter().use { adapter ->
            ...
            assertThat(actions.map { it.title }).contains("Organize imports", "Remove unused import 'ArrayList'")
                .doesNotContain("Remove unused import 'Buffer'")
            actions.forEach { action ->
                val edit = requireNotNull(action.edit)
                assertThat(edit.versioned).isTrue()
                val changed = edit.changes.getValue(uri).sortedWith(compareByDescending<TextEdit> { it.range.start.line }
                    .thenByDescending { it.range.start.column }).fold(text) { value, replacement ->
                    fun offset(position: Position): Int = value.splitToSequence("\n").take(position.line).sumOf { it.length + 1 } + position.column
