...
class TemplateScanner {
    ...
    private fun step(source: CharSequence, state: ScanState): Step = when (state) {
    ...
    private fun scanTemplateStart(source: CharSequence, state: ScanState.Normal): Step {
        val pos = state.pos
    ...
    private fun scanExpression(source: CharSequence, state: ScanState.Expr): Step {
        val pos = state.pos
    ...
    private fun skipNestedString(source: CharSequence, state: ScanState.Expr, delim: Char): Step {
        val pos = state.pos
        if (pos >= source.length) {
    ...
    private fun skipCharLiteral(source: CharSequence, state: ScanState.Expr): Step {
        val pos = state.pos
        if (pos >= source.length) {
    ...
    private fun skipComment(source: CharSequence, state: ScanState.Expr): Step {
        val pos = state.pos
        ...
        return when (source[pos + 1]) {
            '/' -> {
                ...
                val end = (pos + 2 until source.length)
    ...
    private fun unescape(ch: Char): String = when (ch) {
        '{' -> "{"
        '\\' -> "\\"
        '"' -> "\""
        'n' -> "\n"
