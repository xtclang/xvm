    ...
    fun format(
            ...
            (start..minOf(end, lines.lastIndex))
                .flatMap { line ->
                    ...
                    fun depth(
                        ...
                        (
                            before.count { it.id == open } - before.count { it.id == close } -
                                onLine
                                    .takeWhile {
                    ...
                    val indent =
                        depth(Token.Id.L_CURLY, Token.Id.R_CURLY) * config.indentSize +
                            if (depth(Token.Id.L_PAREN, Token.Id.R_PAREN) + depth(Token.Id.L_SQUARE, Token.Id.R_SQUARE) >
                                0
                            ) {
        ...
        val proposed =
            edits
                ...
                .fold(
                    text,
                ) { result, edit -> result.replaceRange(offset(text, edit.range.start), offset(text, edit.range.end), edit.newText) }
            ...
            .flatMap { host ->
                urls
                    ...
                    .map { match ->
                        ...
                        DocumentLink(
                            Range(XdkRename.position(text, start), XdkRename.position(text, start + target.length)),
                            target,
                            target,
                        )
                    }.toList()
        ...
        lex(text).orEmpty().flatMap { token ->
            val kind =
                when {
                    ...
                    token.id != Token.Id.IDENTIFIER && token.id.TEXT
                        ?.firstOrNull()
