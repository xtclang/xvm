...
internal object XdkImports {
    ...
    fun candidates(text: String): List<Candidate> {
        ...
        val imports =
            nodes(root)
                ...
                .filter { !it.isWildcard && it.childNodes().none() }
                .mapNotNull { node ->
        ...
        val reorder =
            groups.mapNotNull { group ->
                if (group.size < 2 ||
                    group.zipWithNext().any { (a, b) -> text.substring(a.second.end, b.second.start).any { !it.isWhitespace() } }
                ) {
    ...
    private fun offset(
        text: String,
        position: Long,
    ): Int? = XdkRename.offset(text, SemanticModel.Position(Source.calculateLine(position), Source.calculateOffset(position)))
