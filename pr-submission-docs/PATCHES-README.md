...
This directory contains 7 self-contained git patches for the code modernization work. Each patch focuses on a single transformation type and can be applied independently to `master`.

## Patch Files

| Patch | Lines | Files | Description |
|-------|-------|-------|-------------|
| `PR-01-collections-empty-singleton.patch` | 673 | 27 | `Collections.emptyList/singletonList()` → `List.of()` |
| `PR-02-arrays-aslist.patch` | 248 | 11 | `Arrays.asList()` → `List.of()` |
| `PR-03-stringbuilder-var.patch` | 2528 | 122 | `StringBuilder sb =` → `var sb =` |
| `PR-04-boolean-first-loop.patch` | 1160 | 24 | Boolean first loop → `Collectors.joining()` |
| `PR-05-arraycopy.patch` | 92 | 4 | `System.arraycopy()` → `Arrays.copyOf()` |
| `PR-06-lazy-list.patch` | 445 | 10 | Lazy `List<X> x = null` → upfront `new ArrayList<>()` |
| `PR-07-loop-to-lambda.patch` | 226 | 5 | Traditional loops → Stream API |
...
6. **PR-04** (Boolean First) - Most transformation complexity
7. **PR-02** (Arrays.asList) - Review for immutability implications
