...
class Parser
    ...
    @Abstract static const AstNode
        {
        @Abstract @RO TextPosition start;
        @Abstract @RO TextPosition end;
        }

    static const Parameter(TypeExpression type,
                           Token?         name  = Null,
    ...
    @Abstract static const Expression
            extends AstNode;

    static const UnaryExpression(Token op, Expression expr)
            extends Expression
        {
        @Override
        TextPosition start.get()
    ...
    static const LiteralExpression(Token value)
            extends Expression
        {
        @Override
        TextPosition start.get()
    ...
    static const LabeledExpression(Token label, Expression expr)
            extends Expression
        {
        @Override
        TextPosition start.get()
    ...
    static const NonBindingExpression(TypeExpression? type,
                                      TextPosition    start,
                                      TextPosition    end)
            extends Expression
        {
    ...
    static const UnionTypeExpression(TypeExpression left, Token operator, TypeExpression right)
            extends RelationalTypeExpression(left, operator, right);

    static const DifferenceTypeExpression(TypeExpression left, Token operator, TypeExpression right)
            extends RelationalTypeExpression(left, operator, right);

    static const IntersectionTypeExpression(TypeExpression left, Token operator, TypeExpression right)
            extends RelationalTypeExpression(left, operator, right);

    @Abstract static const PrefixTypeExpression(Token prefix, TypeExpression type)
            extends TypeExpression
        {
        @Override
        TextPosition start.get()
    ...
    static const ImmutableTypeExpression(Token prefix, TypeExpression type)
    ...
    @Abstract static const SuffixTypeExpression(TypeExpression type, Token suffix)
            extends TypeExpression
        {
        @Override
        TextPosition start.get()
    ...
    static const AccessTypeExpression(TypeExpression type, Token suffix)
    ...
    static const NullableTypeExpression(TypeExpression type, Token suffix)
            extends SuffixTypeExpression(type, suffix);

    static const SequenceTypeExpression(TypeExpression type, Token suffix)
            extends SuffixTypeExpression(type, suffix);

    static const FunctionTypeExpression(Token             func,
                                        Parameter[]       returns,
     ...
     *     Int[]
     */
    static const ArrayTypeExpression(TypeExpression type,
                                     Int            dims,
                                     TextPosition   end)
            extends TypeExpression
        {
     ...
     *     @Unchecked Int
     */
    static const AnnotatedTypeExpression(AnnotationExpression annotation, TypeExpression type)
            extends TypeExpression
        {
        @Override
        TextPosition start.get()
     ...
     *     ecstasy.collections.HashMap!<String?, IntLiteral>
     */
    static const NamedTypeExpression(Token[]           names,
                                     Token?            access,
                                     Token?            noNarrow,
                                     TypeExpression[]? params,
                                     TextPosition      end)
     ...
     */
    static const ChildTypeExpression(TypeExpression          parent,
                                     AnnotationExpression[]? annotations,
                                     Token[]                 names,
                                     TypeExpression[]?       params,
                                     TextPosition            end)
    ...
    static const TupleTypeExpression(TypeExpression[]       types,
                                     TextPosition           start,
                                     TextPosition           end)
            extends TypeExpression
        {
