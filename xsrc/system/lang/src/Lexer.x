...
class Lexer
    ...
    enum Id(String? text, Category category=Normal)
         ...
         */
        static @Lazy Map<String, Id> keywords.calc()
            {
         ...
         */
        static @Lazy Map<String, Id> allKeywords.calc()
            {
            HashMap<String, Id> map = new HashMap();
            for (Id id : values)
                {
                String text = id.text;
                if (id.category != Artificial && (text[0].category.letter || text[0] == '_'))
                    {
                    map[text] = id;
                    }
                }
         ...
         */
        static @Lazy Map<String, Id> prefixes.calc()
            {
            HashMap<String, Id> map = new HashMap();
            for ((String text, Id id) : allKeywords)
                {
