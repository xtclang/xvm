...
abstract class ModifyScriptsTask : DefaultTask() {
    ...
    @get:Input
    abstract val enableNativeAccess: Property<Boolean>
    ...
    @get:OutputFiles
    abstract val outputScripts: ConfigurableFileCollection
    
    @TaskAction
    fun modifyScripts() {
        val outputDir = outputScripts.files.first().parentFile
...
val modifyScripts by tasks.registering(ModifyScriptsTask::class) {
    ...
    enablePreview.set(project.findProperty("org.xtclang.java.enablePreview") as Boolean? ?: true)
    enableNativeAccess.set(project.findProperty("org.xtclang.java.enableNativeAccess") as Boolean? ?: true)
    javaToolsFiles.from(configurations.getByName("xdkJavaTools"))
    
    // Input: Original scripts generated by startScripts task
    inputScripts.from(tasks.startScripts.map { task ->
        listOf("xcc", "xec", "xtc").flatMap { script ->
            listOf(File(task.outputDir!!, script), File(task.outputDir!!, "$script.bat"))
        }
    })
    
    // Output: Same scripts but modified in-place
    outputScripts.from(tasks.startScripts.map { task ->
        listOf("xcc", "xec", "xtc").flatMap { script ->
            listOf(File(task.outputDir!!, script), File(task.outputDir!!, "$script.bat"))
        }
    })
