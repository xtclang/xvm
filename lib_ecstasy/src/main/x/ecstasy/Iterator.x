...
interface Iterator<Element> {
     ...
     *
     * @return True iff the iterator is known to be empty or exhausted; False if the iterator is not
     *         known to be empty (which means that it still _could_ be empty)
     */
    Boolean knownEmpty() {
        if (Int size := knownSize()) {
            return size == 0;
