/**
 * A Weak is used to make a reference into a weak reference. A weak reference is a reference
 ...
 *
 * In order to avoid the possibility of an unassigned reference becoming visible, the Weak must
 * be of a `Referent` that has a default value (such as [Nullable], with its default value of
 * `Null`).
 *
 * A Weak can have a `notify` notification function provided in its construction that is
 ...
 */
annotation Weak<Referent>(function void ()? notify)
        into Var<Referent> {
}
