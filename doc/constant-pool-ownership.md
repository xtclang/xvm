...
Status: implemented on `lagergren/constant-pool-ownership`, reviewed on 2026-09-24.
The review scopes have not yet been extracted into independent branches. This is the companion to the
[embedding plan](../plugin/doc/plans/embedded-runtime-plan.md#constant-pool-ownership-audit) and
[PR extraction plan](../plugin/doc/plans/embedded-runtime-pr-plan.md#constant-pool-audit-follow-up-boundaries).
...
constant-pool fixes. Resource ownership has its own [audit](embedding-resource-ownership.md).
...
JIT implementation work was removed by `d8f8b66a9` and preserved on
[archive/embedded-jit-ownership](https://github.com/xtclang/xvm/tree/archive/embedded-jit-ownership),
with the handoff documented by `ee8a31523`. See [the JIT handoff](jit-embedding.md).
CP-A4, JIT callable-type caches and generated-name resets belong there. They are not fixed by
this interpreter/compiler audit. The embedding JIT API remains an explicit unsupported entry
point on this branch.
...
- Embedding compilation/preparation enters with an empty compatibility binding and retains its
  existing synchronization. Compiler and native bootstrap phases establish their own scopes.
...
concurrent mutation. The embedding compilation lock and serialized preparation remain in place.
...
the resource-lifecycle prerequisite.
...
`ownership/Singletons.x`, run twice as fresh requests in one session, checks child/parent/sibling/
...
`EmbeddingListenerContractTest` checks null rejection and reported failures; existing lifecycle,
compiler and tool suites cover the normal paths. New final-listener source changes in shared
classes do not reintroduce the extracted JIT implementation. A PROBE replacement in a shared
...
Validation recorded on 2026-09-24 before the additional thread-scope test/guard:

- Full javatools: 543 discovered, **503 passed**, 40 existing disabled/skipped, no failures/errors.
- XDK: **36 passed**, no failures/errors/skips: ownership 8, signature 8, lifecycle 11, resource 9.
  The strengthened singleton `.x` fixture was rerun afterward and passed. A later filtered run
  overwrites the XML directory; the combined 36-case result is this recorded earlier execution.
- Plugin: **26 passed**, no skips in its existing result; the last combined invocation reused that
  unchanged task rather than executing it again.
- DIRECT manual tests: **21 modules passed**; manual xUnit: **19 tests passed**. No JIT suite added.
- `spotlessCheck` and `git diff --check` passed.

The thread follow-up first ran ConstantPoolScopeTest against the old close action: 5 cases,
1 failure (the cross-thread close did not throw), zero skips. After adding the creator-thread
guard, the full javatools suite and `spotlessCheck` were rerun: **546 discovered, 506 passed,
40 existing disabled/skipped, zero failures/errors**, with all 5 scope cases executed and passed.
These counts were checked in JUnit XML. The earlier XDK/manual results are not claimed as new
executions after this small scope guard.
...
working tree does not establish that each intermediate extraction builds independently.
...
These limits must stay visible in the submission plan. None warrants describing an observed
...
to their linked source roots. It includes the two original ownership commits and new local files;
...
This inventory accounts for **109 production files and 27 test/resource files**.
Existing supporting tests named in the catalogue but unchanged by these ownership commits are
not counted as newly modified tests. Documentation changes are this file and the two linked
embedding plans. JIT removal/archive documentation is a separate, already committed boundary.
