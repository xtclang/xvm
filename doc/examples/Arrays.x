/**
 * Examples showing how to use arrays in Ecstasy.
 */
module Arrays {
    ...
    void run() {
        ...
        show($"{literalArray=}, {&literalArray.type=}");
        ...
        show($"{biggerArray=}, {&biggerArray.type=}");

        immutable Int[] biggestArray = biggerArray + biggerArray;
        show($"{biggestArray=}, {&biggestArray.type=}");
        ...
        show($|{constantToMutable=}, {&constantToMutable.type=},\
              | {constantToMutable.mutability=}
            );
        Int[] mutableToFixed = biggestArray.toArray(Fixed);
        show($|{mutableToFixed=}, {&mutableToFixed.type=},\
              | {mutableToFixed.mutability=}
            );
        Int[] fixedToPersistent = mutableToFixed.toArray(Persistent);
        show($|{fixedToPersistent=}, {&fixedToPersistent.type=},\
              | {fixedToPersistent.mutability=}
            );
        Int[] persistentToConstant = fixedToPersistent.toArray(Constant);
        show($|{persistentToConstant=}, {&persistentToConstant.type=},\
              | {persistentToConstant.mutability=}
            );

        // an array slice is also an array; this is very handy
        Int[] slice = constantToMutable[1..2];
        show($"{slice=}");

        // slices can be inclusive (by default) or exclusive
        show($"{constantToMutable[1..3]=}");
        show($"{constantToMutable[1>..3]=}");
        show($"{constantToMutable[1..<3]=}");
        show($"{constantToMutable[1>..<3]=}");
