...
A constant asks its own pool - one hop to a real owner. (The no-argument
`ensureTypeInfo()` that made this implicit is since deleted; the pool is still the owner for
the soft asserts that log through it directly.)
...
`getConstantPool()` - and therefore the listener a constant's diagnostics resolve to - is the *compiling*
...
*Updated 2026-09-08. Most of what this section listed is now done; what it says below is measured, not
carried over.*

- **`System.err`: 17 occurrences, and 5 of them are commented out.** So twelve live sites, down from
  the 53 this document started with - and the shrinkage is as much re-counting as fixing. Classifying
  them (E47 step 1) found the earlier figures had been counting dead code and legitimate console
  output as defects.

  Of the twelve, eight are correct as they are and should stay: `RuntimeErrorListener` (printing *is*
  its job), `TypeInfoTrace` (opt-in under `-Dxvm.typeinfo.trace`), `xRTServer` x3 (flag-guarded
  handshake tracing), `tool/Console`, `javajit/Ctx.log` (the debug hook called from generated
  bytecode), and `DebugConsole` (interactive debugger output). `ConstantPool`'s static initializer
  genuinely has no owner - it runs before any pool exists - and already throws with the evidence.

  The three that were real diagnostics have been fixed. `Expression.finishValidations` and
  `ConvertExpression.create` printed "No conversion found for X", which the line above them shows is
  not an error at all - the compiler continues with run-time conversion - so they report
  `COMPILER-209` at INFO. `NameExpression.generateArgument` printed a TODO marker on **every**
  occurrence and now reports `COMPILER-NI`, which already existed for exactly that.

  `Container.recordRuntimeFailure` also changed: it captured a VM defect for `join()` to rethrow and
  printed it, so an embedding host could not observe it at all. It now reports through the
  container's own listener as `RUNTIME-20` as well. The stack trace still goes to `stderr`, because a
  listener carries a message and not a `Throwable`, and for a VM defect the trace is the part worth
  keeping.

- **No-argument `ensureTypeInfo()`: zero. The overload is deleted.** This section previously argued
  that the 60 calls were "the answer, not a backlog", on the grounds that adoption makes the asker
  the owner. That argument was right about safety and wrong about the remedy.

  The rule §8.4 states - *is this call asserting that the type is valid, or asking whether it is?* -
  turns out to answer almost all of them, and "asking" has a spelling that needs no listener at all:
  `typeInfo()`. Of 34 non-`javajit` call sites, 27 were questions and became one-line changes with no
  parameter and no cascade; 3 genuinely asserted and were threaded from callers that already held a
  listener; 4 were left. The 21 `javajit` sites name `ErrorListener.RUNTIME` explicitly, which for a
  module loaded at run time is what the ambient value already was - and for a module compiled in
  process is a fix, because a JIT builder was reporting into a compile that had already finished.

  So the invariant is now unconditional, and it cost classification rather than seventy conversions.
  `ConstantAdoptionListenerTest` still pins adoption, which is what the remaining
  `log(pool.getErrorListener(), ...)` soft asserts rest on.

- **`BLACKHOLE` split into `PROBE` and `BLACKHOLE`.** All 149 uses were classified; one was wrong
  (an `==` against the sentinel in `Compiler.isAbortDesired`, which could never change the answer)
  and is deleted. Of the rest, 136 now say `PROBE` - "I am asking a question, and the asking must not
  be audible" - and 13 keep `BLACKHOLE`, meaning "no caller sink is attached". Same behaviour, and
  `grep PROBE` is now the list of the compiler's speculative paths, which the tree did not previously
  record anywhere.

- **19 empty `catch` blocks remain, audited individually.** Each is recorded as correct in E47:
  `Parser`'s three speculative parses sit inside `try (SafeLookAhead ...)`, so position restore is
  the resource's job; `closeQuietly` does what it says; the diagnostic helpers must not throw while
  describing a failure; the rest are best-effort probes returning null. The catch *parameters* are
  now `_` rather than `ignore`, which the tree already used in `BuildInfo`, so a genuinely ignored
  exception is visually distinct from one named because it gets used.

  Four were defects and are fixed: `xRTNameService` caught `Throwable` in a DNS continuation (an
  `Error` reached XTC code as "host not found") and discarded an interrupt without restoring the
  flag; `KeyStoreOperations.deleteKeyStoreEntry` swallowed a failed delete, so a certificate
  revocation reported success with the revoked certificate still in the store;
  `Launcher.showSystemVersion` relied on catching an NPE to mean "not in the repository"; and
  `Disassembler.printNode` caught `Exception` around a date parse, covering both the parse failure it
  meant and an unrelated NPE.
