...
- adopted singleton constants start with fresh owner-local runtime state,
...
- Annotation templates: no mutable `INSTANCE` fields remain in the annotation
  template package.
- Number templates: no mutable `INSTANCE` fields remain in the number template package.
  Checked integer peer-template globals, unchecked integer/nibble globals, and
