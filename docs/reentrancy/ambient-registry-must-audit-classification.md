...
| JIT bridge ambient helper API (`nObject.$ctx()`, `$xvm()`, `$owner()`) | MUST AUDIT | Hidden current-`Ctx` lookup is usable from any bridge method; `$owner()` combines current XVM with an object-encoded owner id. Static uses are must-fix; ordinary method uses need explicit `Ctx` proof. |
