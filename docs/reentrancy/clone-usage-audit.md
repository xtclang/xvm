...
| --- | --- | --- |
| `Token`, `Source` (compiler) | Trivial: `Token.clone()` is a pure shallow copy of a small value class; `Source.clone()` is shallow + `reset()` (`Token.java:433`, `Source.java:461`). Two copy constructors, one sitting each. | **Retired 2026-08-24** (commit `25371b397`): copy constructors, both classes `final`. The rework surfaced a master defect the mechanism itself caused: `MethodStructure.Source` is a non-static inner class, and `Object.clone()` copies the hidden outer reference, so a cloned method's source stayed bound to the ORIGINAL method - its `getConstantPool()` calls resolved through the wrong owner after `cloneBody` (lambda copies, adoption). The copy constructor re-binds the outer via `that.new Source(...)`, which `clone()` cannot express. Proof: full suite plus `xdk:installDist`, which lexes/parses every Ecstasy module through these classes. |
