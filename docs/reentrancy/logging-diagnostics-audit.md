...
| Test pass/fail authority | `manualTests/build.gradle.kts:592` to `:594` | Done in this branch (verification): every runner path is machine-visible. The reworked `runner.x` completes its result future exceptionally and abandons remaining modules instead of printing and continuing, and the `FailProbe` module proves a runtime failure reaches the Gradle build as a non-zero exit and failed task through (1) the Runner async future chain, (2) a missing-module load, (3) a direct single-module run, and (4) the same-JVM DIRECT executor. The stale build-script TODOs are replaced with the verified contract; `TestAnnotations` passes again and is re-included in the sequential/stress sets. Remaining nicety (tracked as a TODO, not must-fix): xUnit integration so expected-failure tests are first-class. |
