...
| --- | --- | --- |
| Explicit adoption hook | `AllCondition`, `AnyCondition`, `DynamicFormalConstant`, `FPNConstant`, `FSNodeConstant`, `FileStoreConstant`, `Float128Constant`, `FormalTypeChildConstant`, `HandleConstant`, `MethodBindingConstant`, `MethodConstant`, `NamedCondition`, `NotCondition`, `ParameterizedTypeConstant`, `PresentCondition`, `PropertyConstant`, `RegisterConstant`, `SignatureConstant`, `SingletonConstant`, `TypeParameterConstant`, `UInt8ArrayConstant`, `VersionMatchesCondition`, `VersionedCondition` | Fixed or guarded in this branch. These were the known runtime/helper/scratch-state cases where base shallow clone was not acceptable. They now implement `copyForAdoption(...)` under the final wrapper and use explicit construction for adoption rather than the transitional clone helper. |
...
| Value constants inheriting base clone | `ArrayConstant`, `BFloat16Constant`, `ByteConstant`, `CharConstant`, `DecimalAutoConstant`, `DecimalConstant`, `Float16Constant`, `Float32Constant`, `Float64Constant`, `Float8e4Constant`, `Float8e5Constant`, `FloatConstant`, `IntConstant`, `LiteralConstant`, `MapConstant`, `MatchAnyConstant`, `RangeConstant`, `RegExConstant`, `StringConstant`, `ValueConstant`, `VersionConstant` | Current fields are logical values plus transient disassembly indexes or derived primitive arrays. They appear owner-logical under the branch validator, but the default remains brittle because adding one runtime/helper field silently changes adoption behavior. |
