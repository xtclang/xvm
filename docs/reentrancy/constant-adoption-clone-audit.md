...
| Identity, named, and pseudo constants inheriting base clone | `ChildClassConstant`, `ClassConstant`, `DecoratedClassConstant`, `DeferredValueConstant`, `DynamicFormalConstant`, `ExpressionConstant`, `FormalConstant`, `FormalTypeChildConstant`, `KeywordConstant`, `ModuleConstant`, `MultiMethodConstant`, `NamedConstant`, `NativeRebaseConstant`, `PackageConstant`, `ParentClassConstant`, `PropertyConstant`, `PureIdentityConstant`, `ThisClassConstant`, `TypedefConstant`, `UnresolvedNameConstant` | Mostly logical owner/path identity. `PropertyConstant` metadata caches are cleared during registration, but `PropertyConstant.m_sJitName` remains a JIT-owner cache that should be cleared or keyed by JIT `TypeSystem` in the JIT ownership PR. |
