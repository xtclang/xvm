...
| --- | --- | --- |
| Explicit adoption hook | `AccessTypeConstant`, `AllCondition`, `AnyCondition`, `ArrayConstant`, `BFloat16Constant`, `ByteConstant`, `CharConstant`, `DecimalAutoConstant`, `DecimalConstant`, `DynamicFormalConstant`, `FPNConstant`, `FSNodeConstant`, `FileStoreConstant`, `Float128Constant`, `Float16Constant`, `Float32Constant`, `Float64Constant`, `Float8e4Constant`, `Float8e5Constant`, `FormalTypeChildConstant`, `HandleConstant`, `ImmutableTypeConstant`, `IntConstant`, `LiteralConstant`, `MapConstant`, `MatchAnyConstant`, `MethodBindingConstant`, `MethodConstant`, `NamedCondition`, `NotCondition`, `ParameterizedTypeConstant`, `PresentCondition`, `PropertyConstant`, `RangeConstant`, `RegExConstant`, `RegisterConstant`, `ServiceTypeConstant`, `SignatureConstant`, `SingletonConstant`, `StringConstant`, `TerminalTypeConstant`, `TypeParameterConstant`, `UInt8ArrayConstant`, `VersionConstant`, `VersionMatchesCondition`, `VersionedCondition` | Fixed or guarded in this branch. These were known runtime/helper/scratch-state cases, array-backed values, immutable scalar values, composite value containers, parsed-value caches, type-keyed sentinel shells, type leaves, or single-child type wrappers where adoption should be explicit rather than inherited from the transitional clone helper. They now implement `copyForAdoption(...)` under the final wrapper and use explicit construction for adoption. |
| Type constants using common `TypeConstant.setContaining(...)` reset | `AbstractDependantChildTypeConstant`, `AbstractDependantTypeConstant`, `AnnotatedTypeConstant`, `AnonymousClassTypeConstant`, `CastTypeConstant`, `DifferenceTypeConstant`, `InnerChildTypeConstant`, `IntersectionTypeConstant`, `PendingTypeConstant`, `PropertyClassTypeConstant`, `RecursiveTypeConstant`, `RelationalTypeConstant`, `TypeSequenceTypeConstant`, `UnionTypeConstant`, `UnresolvedTypeConstant`, `VirtualChildTypeConstant` | Common type-info, relation, recursion, normalized-type, and JIT-name caches are cleared by `TypeConstant.setContaining(...)`. Subclass transient fields are mostly disassembly indexes. `AnnotatedTypeConstant.m_typeAnno` is cleared by `registerConstants(...)`, which is acceptable only if registration remains single-owner or becomes transactionally published. `TerminalTypeConstant`, `AccessTypeConstant`, `ImmutableTypeConstant`, and `ServiceTypeConstant` have moved to explicit construction and foreign child/identity rejection. |
