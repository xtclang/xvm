...
| --- | --- | --- |
| Explicit adoption hook | `AllCondition`, `AnyCondition`, `ArrayConstant`, `BFloat16Constant`, `ByteConstant`, `CharConstant`, `DecimalConstant`, `DynamicFormalConstant`, `FPNConstant`, `FSNodeConstant`, `FileStoreConstant`, `Float128Constant`, `Float16Constant`, `Float32Constant`, `Float64Constant`, `Float8e4Constant`, `Float8e5Constant`, `FormalTypeChildConstant`, `HandleConstant`, `IntConstant`, `MapConstant`, `MethodBindingConstant`, `MethodConstant`, `NamedCondition`, `NotCondition`, `ParameterizedTypeConstant`, `PresentCondition`, `PropertyConstant`, `RangeConstant`, `RegExConstant`, `RegisterConstant`, `SignatureConstant`, `SingletonConstant`, `StringConstant`, `TypeParameterConstant`, `UInt8ArrayConstant`, `VersionMatchesCondition`, `VersionedCondition` | Fixed or guarded in this branch. These were known runtime/helper/scratch-state cases, array-backed values, immutable scalar values, or composite value containers where adoption should be explicit rather than inherited from the transitional clone helper. They now implement `copyForAdoption(...)` under the final wrapper and use explicit construction for adoption. |
...
| Value constants still inheriting base clone | `DecimalAutoConstant`, `FloatConstant`, `LiteralConstant`, `MatchAnyConstant`, `ValueConstant`, `VersionConstant` | These are mutable-cache, abstract-family, type-locator, or parsed-value-cache shapes. They appear owner-logical under the branch validator, but the default remains brittle because adding one runtime/helper field silently changes adoption behavior. `MatchAnyConstant` also exposes the unresolved type-family/locator adoption problem and should be fixed with the type-family slice. |
