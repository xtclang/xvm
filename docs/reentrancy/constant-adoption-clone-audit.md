...
| --- | --- | --- |
| Explicit adoption hook | `DynamicFormalConstant`, `FSNodeConstant`, `FileStoreConstant`, `FormalTypeChildConstant`, `HandleConstant`, `MethodConstant`, `ParameterizedTypeConstant`, `PropertyConstant`, `RegisterConstant`, `SignatureConstant`, `SingletonConstant`, `TypeParameterConstant` | Fixed or guarded in this branch. These were the known runtime/helper-state cases where base shallow clone was not acceptable. They now implement `copyForAdoption(...)` under the final wrapper and use explicit construction for adoption rather than the transitional clone helper. |
...
| Frame-dependent constants | `MethodBindingConstant`, plus explicit `RegisterConstant` and guarded `HandleConstant` | `MethodBindingConstant` is a serialized logical frame-dependent form. `RegisterConstant` has an explicit hook because the in-memory compiler form can carry a transient `Register`; `HandleConstant` is guarded because it wraps a live `ObjectHandle`. |
