...
| Identity, named, and pseudo constants inheriting base clone | `ChildClassConstant`, `ClassConstant`, `DecoratedClassConstant`, `DeferredValueConstant`, `DynamicFormalConstant`, `ExpressionConstant`, `FormalConstant`, `KeywordConstant`, `ModuleConstant`, `MultiMethodConstant`, `NamedConstant`, `NativeRebaseConstant`, `PackageConstant`, `ParentClassConstant`, `PureIdentityConstant`, `ThisClassConstant`, `TypedefConstant`, `UnresolvedNameConstant` | Mostly logical owner/path identity. `DynamicFormalConstant` remains the most suspicious item in this bucket because it can carry compiler register state; it should be the next identity/formal adoption hook. |
