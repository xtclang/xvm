...
| --- | --- | --- |
| Explicit adoption hook | `AccessTypeConstant`, `AllCondition`, `AnyCondition`, `ArrayConstant`, `BFloat16Constant`, `ByteConstant`, `CastTypeConstant`, `CharConstant`, `DecimalAutoConstant`, `DecimalConstant`, `DifferenceTypeConstant`, `DynamicFormalConstant`, `FPNConstant`, `FSNodeConstant`, `FileStoreConstant`, `Float128Constant`, `Float16Constant`, `Float32Constant`, `Float64Constant`, `Float8e4Constant`, `Float8e5Constant`, `FormalTypeChildConstant`, `HandleConstant`, `ImmutableTypeConstant`, `IntConstant`, `IntersectionTypeConstant`, `LiteralConstant`, `MapConstant`, `MatchAnyConstant`, `MethodBindingConstant`, `MethodConstant`, `NamedCondition`, `NotCondition`, `ParameterizedTypeConstant`, `PresentCondition`, `PropertyConstant`, `RangeConstant`, `RegExConstant`, `RegisterConstant`, `ServiceTypeConstant`, `SignatureConstant`, `SingletonConstant`, `StringConstant`, `TerminalTypeConstant`, `TypeParameterConstant`, `UInt8ArrayConstant`, `UnionTypeConstant`, `VersionConstant`, `VersionMatchesCondition`, `VersionedCondition` | Fixed or guarded in this branch. These were known runtime/helper/scratch-state cases, array-backed values, immutable scalar values, composite value containers, parsed-value caches, type-keyed sentinel shells, type leaves, single-child type wrappers, storable relational types, or transient type markers where adoption should be explicit rather than inherited from the transitional clone helper. They now implement `copyForAdoption(...)` under the final wrapper and use explicit construction or fail closed. |
| Type constants using common `TypeConstant.setContaining(...)` reset | `AbstractDependantChildTypeConstant`, `AbstractDependantTypeConstant`, `AnnotatedTypeConstant`, `AnonymousClassTypeConstant`, `InnerChildTypeConstant`, `PendingTypeConstant`, `PropertyClassTypeConstant`, `RecursiveTypeConstant`, `RelationalTypeConstant`, `TypeSequenceTypeConstant`, `UnresolvedTypeConstant`, `VirtualChildTypeConstant` | Common type-info, relation, recursion, normalized-type, and JIT-name caches are cleared by `TypeConstant.setContaining(...)`. Subclass transient fields are mostly disassembly indexes. `AnnotatedTypeConstant.m_typeAnno` is cleared by `registerConstants(...)`, which is acceptable only if registration remains single-owner or becomes transactionally published. `TerminalTypeConstant`, `AccessTypeConstant`, `ImmutableTypeConstant`, `ServiceTypeConstant`, `UnionTypeConstant`, `IntersectionTypeConstant`, `DifferenceTypeConstant`, and `CastTypeConstant` have moved to explicit construction or fail-closed adoption. |
| Identity, named, and pseudo constants inheriting base clone | `ChildClassConstant`, `ClassConstant`, `DecoratedClassConstant`, `DeferredValueConstant`, `ExpressionConstant`, `FormalConstant`, `KeywordConstant`, `ModuleConstant`, `MultiMethodConstant`, `NamedConstant`, `NativeRebaseConstant`, `PackageConstant`, `ParentClassConstant`, `PureIdentityConstant`, `ThisClassConstant`, `TypedefConstant`, `UnresolvedNameConstant` | Mostly logical owner/path identity. `DynamicFormalConstant` was removed from this bucket because it carried compiler register state and needed an explicit hook. |
