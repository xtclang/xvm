...
| --- | --- | --- |
| Explicit adoption hook | `DynamicFormalConstant`, `FSNodeConstant`, `FileStoreConstant`, `FormalTypeChildConstant`, `HandleConstant`, `MethodBindingConstant`, `MethodConstant`, `ParameterizedTypeConstant`, `PropertyConstant`, `RegisterConstant`, `SignatureConstant`, `SingletonConstant`, `TypeParameterConstant` | Fixed or guarded in this branch. These were the known runtime/helper-state cases where base shallow clone was not acceptable. They now implement `copyForAdoption(...)` under the final wrapper and use explicit construction for adoption rather than the transitional clone helper. |
