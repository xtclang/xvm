...
| --- | --- | --- |
| Explicit adoption hook | `AllCondition`, `AnyCondition`, `BFloat16Constant`, `ByteConstant`, `CharConstant`, `DecimalConstant`, `DynamicFormalConstant`, `FPNConstant`, `FSNodeConstant`, `FileStoreConstant`, `Float128Constant`, `Float16Constant`, `Float32Constant`, `Float64Constant`, `Float8e4Constant`, `Float8e5Constant`, `FormalTypeChildConstant`, `HandleConstant`, `IntConstant`, `MethodBindingConstant`, `MethodConstant`, `NamedCondition`, `NotCondition`, `ParameterizedTypeConstant`, `PresentCondition`, `PropertyConstant`, `RegExConstant`, `RegisterConstant`, `SignatureConstant`, `SingletonConstant`, `StringConstant`, `TypeParameterConstant`, `UInt8ArrayConstant`, `VersionMatchesCondition`, `VersionedCondition` | Fixed or guarded in this branch. These were known runtime/helper/scratch-state cases, array-backed values, or immutable scalar values where adoption should be explicit rather than inherited from the transitional clone helper. They now implement `copyForAdoption(...)` under the final wrapper and use explicit construction for adoption. |
...
| Value constants still inheriting base clone | `ArrayConstant`, `DecimalAutoConstant`, `FloatConstant`, `LiteralConstant`, `MapConstant`, `MatchAnyConstant`, `RangeConstant`, `ValueConstant`, `VersionConstant` | These are mutable-cache, abstract-family, or child-constant shapes. They appear owner-logical under the branch validator, but the default remains brittle because adding one runtime/helper field silently changes adoption behavior. They are the next value-family clone-free slices, not part of the scalar wave. |
