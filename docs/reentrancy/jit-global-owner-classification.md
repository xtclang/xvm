...
| JIT exception host and bridge boundaries | DONE for local branch scope | `JitConnector.invoke0Impl(...)` detects generated unhandled exceptions; `nType` calls generated `equals`, `compare`, and `hashCode` through reflection. | Branch now preserves non-zero `join()` results for unhandled generated exceptions and rethrows invoked `nException`/`Error` from `nType`. Broader JIT owner lifecycle is still separate. |
