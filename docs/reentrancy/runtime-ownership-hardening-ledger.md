...
| 14 | Cross-owner `GenericHandle` masking | Done for the highest-risk `ObjectHandle.cloneAs(...)` subcase. `cloneAs(...)` is an access-view operation over live runtime state, not an owner-transfer primitive. `GenericHandle.maskAs(...)` now rejects direct cross-owner masking unless the whole handle graph is already shared with the target container; non-core objects still use the existing proxy path. | `OwnershipDiagnosticsTest.crossOwnerMaskRejectsNonSharedHandleBeforeClone()` constructs a non-shared synthetic handle that throws if `cloneAs(...)` is reached. The fixed path checks sharing and returns `null` before any shallow clone. Same-owner view backing remains a separate design task because blindly copying fields would break write-through semantics. |
