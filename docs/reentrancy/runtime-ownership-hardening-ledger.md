...
| 9 | Runtime-executed `Op` caches | Done in this branch wave for owner-bearing frame-constant and switch-table caches. `JumpCond`/`JumpNCond` no longer cache `ConditionalConstant` on the op, `OpTest`/`OpCondJump` no longer write frame type constants back to `m_typeCommon` during execution, and `JumpVal`/`JumpVal_N` keep first-execution switch tables in the executing `Container` instead of on the shared decoded op. | `OpRuntimeCacheTest` verifies the condition fields are gone, guards against the old common-type write-back pattern, and fails if decoded switch ops regain owner-bearing handle/type/map fields. Same-JVM sequence and parallel stress exercise these op paths under runtime ownership validation. |
