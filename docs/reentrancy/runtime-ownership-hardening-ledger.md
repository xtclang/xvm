...
| 9 | Runtime-executed `Op` caches | Done in this branch wave for owner-bearing frame-constant and switch-table caches, plus the deliberate decoded-op `assert:once` state. `JumpCond`/`JumpNCond` no longer cache `ConditionalConstant` on the op, `OpTest`/`OpCondJump` no longer write frame type constants back to `m_typeCommon` during execution, `JumpVal`/`JumpVal_N` keep first-execution switch tables in the executing `Container`, and `JumpNFirst` uses a final `AtomicBoolean` for process-wide decoded-op once state. | `OpRuntimeCacheTest` verifies the condition fields are gone, guards against the old common-type write-back pattern, fails if decoded switch ops regain owner-bearing handle/type/map fields, and proves `JumpNFirst` has exactly one concurrent first-execution winner. Same-JVM sequence and parallel stress exercise these op paths under runtime ownership validation. |
