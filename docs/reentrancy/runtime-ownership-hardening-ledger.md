...
| 17 | Explicit constant adoption clone policy | Done in this branch wave as a guard. Base `Constant.adoptedBy(...)` is now the final owner-transfer wrapper and no longer lets a new subclass silently inherit shallow `Object.clone()` ownership transfer. The clone-free slices also move `FSNodeConstant`, `FileStoreConstant`, fresh `HandleConstant` registration, `MethodConstant` method/JIT caches, `PropertyConstant` metadata/JIT caches, `DynamicFormalConstant` compiler-register state, `RegisterConstant` compiler-register state, `MethodBindingConstant` frame descriptor state, condition simulation scratch, array-backed byte storage, immutable scalar values, composite value containers, parsed/delegated value caches, the `MatchAnyConstant` wildcard shell, the `TerminalTypeConstant` type shell, and single-child type wrappers to explicit construction instead of clone-then-clear. | `ConstantAdoptionTest.defaultAdoptionCloneRequiresExplicitPolicy()` proves a no-policy subclass fails adoption, `ConstantAdoptionTest.adoptionWrapperIsFinalAndSpecialCasesUseCopyHook()` proves the fixed classes use the hook, and adoption tests prove filesystem, handle, method-cache, property-cache, formal-child, dynamic-formal, register-constant, method-binding, condition scratch, byte-array backing, scalar-value, composite value, literal-cache, version, decimal-auto, match-any shareable type-key behavior, match-any foreign type-key rejection, terminal-type shared identity adoption, terminal-type foreign identity rejection, single-child type-wrapper shared-child adoption, and single-child type-wrapper foreign-child rejection. The clone-free architecture remains tracked separately: every opted-in family should eventually move to explicit copy/adoption constructors. |
