...
| 17 | Explicit constant adoption clone policy | Done in this branch wave as a guard. Base `Constant.adoptedBy(...)` is now the final owner-transfer wrapper and no longer lets a new subclass silently inherit shallow `Object.clone()` ownership transfer. The clone-free slices also move `FSNodeConstant`, `FileStoreConstant`, fresh `HandleConstant` registration, `MethodConstant` method/JIT caches, `PropertyConstant` metadata/JIT caches, `DynamicFormalConstant` compiler-register state, `RegisterConstant` compiler-register state, `MethodBindingConstant` frame descriptor state, condition simulation scratch, array-backed byte storage, immutable scalar values, composite value containers, and parsed/delegated value caches to explicit construction instead of clone-then-clear. | `ConstantAdoptionTest.defaultAdoptionCloneRequiresExplicitPolicy()` proves a no-policy subclass fails adoption, `ConstantAdoptionTest.adoptionWrapperIsFinalAndSpecialCasesUseCopyHook()` proves the fixed classes use the hook, and adoption tests prove filesystem, handle, method-cache, property-cache, formal-child, dynamic-formal, register-constant, method-binding, condition scratch, byte-array backing, scalar-value, composite value, literal-cache, version, and decimal-auto behavior is preserved or rejected at the boundary when invalid. The clone-free architecture remains tracked separately: every opted-in family should eventually move to explicit copy/adoption constructors. |
