...
| Runtime-published pools need a full freeze model | A container-visible pool now rejects new registrations after the runtime publication marker, but destructive compiler/linker storage rewrites, live handle constants, and storage snapshots are not closed by that marker. Parallel readers still need a clear immutable runtime-pool contract. | Freeze runtime pools after warmup or split mutable compiler/linker pools from immutable runtime pools. Keep post-publication registration failures on runtime paths and close destructive storage phases separately. |
| Clone-free `Constant.adoptedBy(...)` architecture | Fixed in this branch. `Constant` no longer implements `Cloneable`, there is no `cloneForAdoption(...)` or `allowsDefaultAdoptionClone()` fallback, and every concrete constant adoption path now constructs an explicit target-owned logical value, inherits a reviewed hook, or fails closed. Future explicit hooks can still be written incorrectly by reusing owner-local helpers. | Keep the final wrapper, source-shape tests, and `ConstantAdoptionValidator` in stress/CI. Treat any new non-logical field on a `Constant` subclass as requiring an explicit adoption decision. Track remaining clone work under non-constant families such as component/method metadata and runtime handle views. |
