...
| `JumpNFirst.m_fVisited` | Follow-up must-fix | `runtime-metadata-op-cache-classification.md`, `must-audit-backlog.md` | A plain boolean on a decoded op makes "first" process-wide by accident. | Define semantics, then test same-JVM and concurrent invocations. |
