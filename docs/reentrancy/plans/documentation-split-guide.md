...
| Constant adoption and clone hazards | Implemented as a guard; clone-free adoption remains follow-up | `constant-adoption-clone-audit.md`, `clone-usage-audit.md`, `stress-discovered-runtime-issues.md`, `plans/clone-free-adoption-plan.md` | Shallow clone preserved runtime state, locks, atomics, thread-local cells, and handles across pool owners. The branch now fails closed unless a family explicitly opts into the transitional clone helper. | Adoption validator, focused adoption tests, no-policy adoption test, and `TestProps` parallel stress with ownership validation. |
| ConstantPool registration publication | Implemented as a guard; transactional registration remains follow-up | `constant-pool-must-audit-classification.md`, `constant-pool-hostile-state-audit.md`, `bad-design-decisions-reference.md` | `register(...)` used one public list/map as both private recursion worklist and completed constant storage. The branch preserves same-thread recursion but blocks public cross-thread observation until completion. | Same-thread reentrancy test, cross-thread wait test, failed-registration reader test, and later transaction worklist proof. |
