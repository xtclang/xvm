...
| L20 | Native injection duplicate-service races (`fix/native-injection-singletons`) | 2026-08-25 fix wave: DCL for synchronous sites, resolved-only first-wins publish + loser terminate for construct-request sites, deferred-caching defect closed; `NativeInjectionSingletonTest` red-verified | Master-verbatim; graduated and fixed 2026-08-25. | S25 | None |
...
| `ConstantPool.f_implicits` plain HashMap written from concurrent service threads (lazy-cache audit graduation) - FIXED `8077ad6c0` | Concurrent, but reachable in ANY program whose services resolve implicit names - single container, shared executor, same bucket as the timer rows | `ConstantPoolDiagnosticsTest.implicitIdentityCacheIsConcurrentSafe()` (red on the HashMap shape, verified by reverting) |
| Delegation synthesis publishes half-built method code before assembly (frozen-code audit graduation) - FIXED `5d9a5f395` | Concurrent, but reachable whenever two services first-dispatch a delegating member - lazy per-TypeInfo-view chain builds race one host ClassStructure | `DelegationSynthesisTest` (concurrent hammer; the deterministic marker red is branch-only) |
