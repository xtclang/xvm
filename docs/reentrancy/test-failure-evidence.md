...
| `org.xvm.runtime.RuntimeFailurePropagationTest` | `MainContainer.invoke0()` still builds `". Cause: " + e.getMessage()` and drops the original exception. | Startup/invocation failure diagnostics lost owner, module-load, stack, suppressed, and cause-chain information before reaching the launcher. |
