...
| `org.xvm.lib.jsondb.JsondbClientRollbackFailureTest` | `Client.RootTransaction.commit(...)` still catches the compensating rollback failure with `catch (Exception ignore) {}` after a commit failure. | jsondb reported an ordinary database error while deleting the evidence that commit failed and rollback also failed, which is the unknown-disposition state DB health/recovery needs to see. |
...
| `org.xvm.runtime.NativeCallbackRegistrationTest` | LocalClock constructor registration, NanoTimer `catch (Throwable)` swallowing, xRTServer missing rollback, and the lazy plain-`HashMap` alarm-callback registry with a throwing `extractCallback()` are still present. | Native keep-alive registration was not exception-safe; schedule/bind failure could strand callback counts; and the callback registry raced the service thread against the shared static Timer thread, whose worst case kills the Timer and silently disables every alarm in every container. |
...
| `org.xvm.asm.constants.HandleConstantAssembleTest` | `HandleConstant.assemble(...)` completes without throwing; the inherited Register `assemble()` writes only the borrowed format byte. | A runtime-only handle constant referenced at serialization time would silently produce a structurally corrupt module instead of failing loudly. Compiles on master unchanged (`HandleConstant(ObjectHandle)` and `ObjectHandle.DEFAULT` both exist there). |
| `org.xvm.runtime.template._native.reflect.MethodInvokeArgumentAliasingTest` | `reflectionInvokePathClonesTupleStorage()` shape pin: `xRTMethod.invokeInvoke` still hands `hTuple.m_ahValue` raw to the call chain (the in-code `TODO GG+CP do we need to check these?` still asks exactly this). | Reflection `Method.invoke` turned the caller's tuple storage into the callee register file; a parameter reassignment mutates the caller's - possibly const-heap-cached - tuple. The `ensureSize` pass-through contract and function-path copy pins document why the clone is load-bearing. |
| `org.xvm.asm.ComponentMethodParameterCopyTest` | `shortHandPropertyPathUsesCopyingFactory()` call-site pin fails: `MethodDeclarationStatement` still builds short-hand property overrides from the super method's own `Parameter` elements. | Short-hand property compilation mutated the loaded library module's shared `Parameter` constants at assembly registration. (The two factory-behavior tests need the additive `Component.createMethodCopyingParameters(...)` present to compile on master.) |
| `org.xvm.asm.ComponentBodyCopyTest` | `contributionsAreReOwnedByBodyCopies()` and `methodBodyCopyRebindsSourceOuter()` fail: cloned contributions answer `getComponent()` with the SOURCE component, and cloned method source resolves `getConstantPool()` through the SOURCE method. | `Object.clone()` on inner classes silently preserved the hidden outer reference, so body copies kept answering with the source owner. (The Cloneable/ratchet/clone-override pins in the same class fail on master because the copy-constructor redesign is branch policy, not master-bug evidence - cite only the two re-owner tests.) |
| `org.xvm.runtime.template._native.reflect.FinalizerChainTest` | `chainAppendsAtTailInsteadOfDroppingLinkedFinalizers()` is behaviorally red under both `-ea` (assert) and `-da` (silent overwrite drops the linked finalizer). | `FullyBoundHandle.chain()` asserted `m_next == null`, which `Frame.chainFinalizer` can legally violate before the construction epilogue folds per-frame finalizers. (Needs a one-line master adaptation: `NativeContainer.create(runtime, repo)` is the branch factory; master constructs `new NativeContainer(runtime, repo)`.) |
...
| `org.xvm.asm.ConstantPoolDiagnosticsTest` | The always-on runtime publication marker and destructive-phase publication guards (branch APIs). `publicationMarkerIsInstalledByDefault()` is red on the old property-gated shape because no marker is installed in normal runtime mode; the `runtimePublishedPoolRejects*` tests are red on the old register-only guard because write/replace/invalidate operations keep mutating after publication. `implicitIdentityCacheIsConcurrentSafe()` is red on the plain-`HashMap` `f_implicits` shape (verified by reverting the fix). `currentPoolLookupGetterDoesNotExist()` pins the deleted ambient-pool family (`getCurrentPool`, `withPool`, `s_tloPool`, both transitional assertions) away by reflection - red on any shape that still declares them. |
...
| `org.xvm.runtime.template.collections.ArrayViewGuardTest` | Rewritten 2026-08-25 from guard pin to sharing proof after the same-JVM stress harness proved the view path ORDINARY (`MOV_THIS_A #1, PRIVATE` on a live mutable `Array<Char>`): views now share one `ArrayState` cell, and the tests assert a delegate swap or freeze through one view is authoritative for all views (delegates still refuse view cloning entirely). Red-verified against a simulated per-view-cell `cloneAs` - the master-equivalent shape, where master's shallow clone forks the delegate on `clear()` and splits the enum on freeze; the freeze-split half is red on master with master-only APIs (`cloneAs` + `setMutability`/`isMutable`). |
