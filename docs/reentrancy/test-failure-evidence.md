...
| `org.xvm.javajit.JitFailurePropagationTest` | Generated unhandled XTC exception branch only prints diagnostics and has `catch (Throwable ignore)`. | JIT/direct execution could report success after an unhandled generated exception if the connector result was already zero, and diagnostic rendering could hide arbitrary VM failures. |
