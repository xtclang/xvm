...
| --- | --- | --- | --- | --- |
| Must fix | Mutable native template `INSTANCE` fields | `master`: 143 mutable template `INSTANCE` fields and 139 constructor assignments. This branch fixes 64 fields / 61 assignments and leaves 79/78. | Last writer wins across containers; constructor `this` escape | `NativeTemplates` central key table plus container/frame lookup |
| Must fix | Static runtime-owned metadata | `master`: 149 field-shaped runtime/template static metadata fields after excluding `INSTANCE`. This branch fixes 54 and leaves 95. | Type/composition/method/handle values from one owner reused in another owner | Owner-scoped final `Lazy`, grouped info records, or owner-owned `ConcurrentMap` |
