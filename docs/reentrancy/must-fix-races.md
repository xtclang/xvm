...
| --- | --- | --- | --- | --- |
| Must fix | Mutable native template `INSTANCE` fields | `master`: 143 mutable template `INSTANCE` fields and 139 constructor assignments. This branch fixes 100 fields / 96 assignments and leaves 43/43. | Last writer wins across containers; constructor `this` escape | `NativeTemplates` central key table plus container/frame lookup |
| Must fix | Static runtime-owned metadata | `master`: 151 field-shaped runtime/template static metadata fields after excluding `INSTANCE`. This branch fixes 73 and leaves 78. | Type/composition/method/handle values from one owner reused in another owner | Owner-scoped final `Lazy`, grouped info records, or owner-owned `ConcurrentMap` |
