...
| Must fix | Shallow-copied transient runtime/helper state during constant adoption | Fixed in this branch for the `Constant` adoption mechanism: `Constant` no longer implements `Cloneable`, the fallback clone helper is gone, and concrete constants use explicit copy hooks or fail closed | A constant adopted into a different pool can carry runtime state, helper locks, JIT caches, reentrancy markers, or live handles owned by the source pool | Keep explicit `copyForAdoption(...)` hooks, source-shape tests, and adoption validation; no owner-transfer clone path should return |
