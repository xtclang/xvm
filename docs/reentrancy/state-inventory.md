...
| --- | --- | --- | --- | --- |
| Must fix | Mutable template `INSTANCE` fields and `INSTANCE = this` constructors | `master`: 143 mutable template `INSTANCE` fields; 139 constructor assignments. This branch fixes 21 and leaves 122/118. | Process-global last-writer-wins template lookup; constructor `this` escape; wrong container/pool can be observed | `NativeTemplates` central key table plus owner-scoped lazy lookup |
...
| Should fix | Constant-looking non-final public statics | 171 public non-final uppercase/static constant-shaped fields | They look immutable in review but can be reassigned and are not safely published as constants | `public static final` immutable values, private owner-scoped state, or accessor methods |
...
- `xOSFile`: publishes `INSTANCE = this`, then `initNative()` writes
  `s_constructor`.
- `xRTCertificateManager`: publishes `INSTANCE = this` and lazily assigns
  `m_typeCanonical` on first canonical-type access.
...
javatools/src/main/java/org/xvm/runtime/template/_native/collections/xBasicHashCollector.java
javatools/src/main/java/org/xvm/runtime/template/_native/crypto/xRTAlgorithms.java
...
javatools/src/main/java/org/xvm/runtime/template/_native/io/xTerminalConsole.java
javatools/src/main/java/org/xvm/runtime/template/_native/lang/src/xRTCompiler.java
javatools/src/main/java/org/xvm/runtime/template/_native/mgmt/xCoreRepository.java
javatools/src/main/java/org/xvm/runtime/template/_native/net/xRTNetwork.java
...
javatools/src/main/java/org/xvm/runtime/template/_native/numbers/xRTRandom.java
...
javatools/src/main/java/org/xvm/runtime/template/_native/web/xRTConnector.java
javatools/src/main/java/org/xvm/runtime/template/_native/web/xRTServer.java
...
javatools/src/main/java/org/xvm/runtime/template/numbers/xInt16.java
...
javatools/src/main/java/org/xvm/runtime/template/numbers/xUInt16.java
javatools/src/main/java/org/xvm/runtime/template/numbers/xUInt32.java
...
javatools/src/main/java/org/xvm/runtime/template/reflect/xInjector.java
...
javatools/src/main/java/org/xvm/runtime/template/xObject.java
```
...
- Structure-owned methods in static fields, for example
  `Utils.RT_PARAMETER_CONSTRUCT`, `xConst.FN_APPEND_TO`, and
  `xOSFile.s_constructor`.
- Runtime handles/constants in static fields, for example
  `xRTFunction.EMPTY_FUNCTION_ARRAY` and `xRTProperty.EMPTY_PROPERTY_ARRAY`.
- Static maps keyed by pool-owned values, for example `xAtomic.NUMBER_TEMPLATES`
  and `xRTViewToBit.VIEWS`.
...
```text
274 runtime/asm/compiler non-final static fields
277 all-Java non-final static fields
```

Representative examples:

- `Utils.CONST_HELPER`, `Utils.ANNOTATION_TEMPLATE`,
  `Utils.RT_PARAMETER_TEMPLATE`, and related method/type caches.
...
```text
171 public non-final uppercase/static constant-shaped fields
```

Representative examples:

- `Compiler.EXPRESSION_UNREACHABLE`.
- `EnumValueBuilder.NAME`, `EnumerationBuilder.NAMES`, and
  `EnumerationBuilder.VALUES`.
- `ClassTemplate.VOID`, `THIS`, `OBJECT`, `INT`, `STRING`, `BOOLEAN`, and
  `BYTES`.
- `Op.NO_ARGS`.
- Template handles and constants such as `xBoolean.TRUE`, `xBoolean.FALSE`,
  `xNullable.NULL`, `xString.EMPTY_STRING`, `xString.EMPTY_ARRAY`, `xString.ZERO`,
  and `xString.ONE`.
