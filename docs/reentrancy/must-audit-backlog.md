...
| Should remove after semantic callers are gone | `ConstantPool.getCurrentPool()` compatibility API | `ambient-context-audit.md`, `scoped-value.md` | Keeping the getter makes new semantic misuse easy even though current call sites are fixed. | The getter is now deprecated for removal; keep `ConstantPoolDiagnosticsTest.semanticCurrentPoolLookupIsBridgeOnly()` and eventually replace `withPool(...)` with explicit owner APIs or a tightly scoped boundary object. |
