...
| Must audit, likely small | Remaining semantic `ConstantPool.getCurrentPool()` outside the bridge | `ambient-context-audit.md`, `constant-pool-hostile-state-audit.md` | After the metadata fix, the remaining non-bridge source call is `FileStructure.getErrorListener()`, which redirects diagnostics through ambient state. | Decide whether diagnostics should use the file owner's listener, an explicit caller listener, or a checked boundary bridge; then add a no-ambient/wrong-ambient test. |
| Should remove after semantic callers are gone | `ConstantPool.getCurrentPool()` compatibility API | `ambient-context-audit.md`, `scoped-value.md` | Keeping the getter makes new semantic misuse easy even if current call sites are fixed. | Make semantic source-shape tests fail new uses, deprecate the getter, and eventually replace `withPool(...)` with explicit owner APIs or a tightly scoped boundary object. |
