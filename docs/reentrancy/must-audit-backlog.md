...
| Split/background complete | Compiler/lexer/parser `this-escape` warnings | `this-escape-tally.md`, background branch `lagergren/fix-compiler-this-escape` | Incremental/parallel compilation cannot rely on parser/compiler constructors dispatching into partially built AST/request objects. | Separate branch commit `9e3253f8a9e2 Fix compiler this-escape construction warnings` removes the compiler/lexer/parser warnings and adds focused tests/docs. Keep it out of this runtime-owner PR unless explicitly rebased. |
