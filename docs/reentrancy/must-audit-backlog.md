...
| Should remove after semantic callers are gone | `ConstantPool.getCurrentPool()` compatibility API | `ambient-context-audit.md`, `scoped-value.md` | Keeping the getter makes new semantic misuse easy even though current call sites are fixed. | Keep `ConstantPoolDiagnosticsTest.semanticCurrentPoolLookupIsBridgeOnly()`, deprecate the getter, and eventually replace `withPool(...)` with explicit owner APIs or a tightly scoped boundary object. |
