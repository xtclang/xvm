...
| Split/background | Compiler/lexer/parser `this-escape` warnings | `this-escape-tally.md`, background branch `lagergren/fix-compiler-this-escape` | Incremental/parallel compilation cannot rely on parser/compiler constructors dispatching into partially built AST/request objects. | Background agent `01a02707-6d20-72f1-be1b-eefb16d9a11b` owns this branch; require before/after lint and focused tests. |
| Must fix next if kept in this branch | Remaining runtime `Container` constructor escapes | `this-escape-removal-audit.md` | `new ConstHeap(this)` and field-initialized `new NativeTemplates(this)` still capture the owner during base construction. | Prefer stateless/owner-explicit `NativeTemplates` and a post-construction or proven-nonpublishing `ConstHeap` path; lint must drop `Container.java` warnings or local proof must be documented. |
| Must fix | `Parameter.cloneBody()` source mutation and copied deref state | `clone-usage-audit.md` | The clone method clears deref state on the source and leaves copied method-owned register state on the clone. | Replace `Object.clone()` with an explicit parameter copy for a target method; prove source/copy deref registers and owners are distinct. |
| Must fix | `MethodStructure.cloneBody()` cloned parameter ownership | `clone-usage-audit.md` | Cloned parameters are assigned back to the original method owner via `param.setContaining(this)`. | Set cloned parameters on `that`; add tests that cloned method params resolve through the cloned method/pool. |
| Must fix unless proven impossible | Delegated method `Parameter[]` element sharing | `clone-usage-audit.md` | Delegated methods clone only parameter array containers and share mutable `Parameter` elements. | Deep-copy parameter elements or prove delegated parameters can never call owner-sensitive `deref(...)`. |
| Must audit, likely must fix | `ObjectHandle.cloneAs(...)` shallow runtime-handle copies | `clone-usage-audit.md` | View/mask clones can share mutable field arrays and rewired `RefHandle` objects; cross-container movement can retain source handles. | Replace with explicit view/copy constructors or assert/freeze/share semantics for retained handles. |
...
| Must fix | `TypeConstant.s_setRecursions` mutable static `HashSet` | `constant-pool-hostile-state-audit.md` | A process-global `HashSet` is mutated during type relation recursion diagnostics; concurrent type checks can corrupt it and it leaks cross-pool diagnostic state. | Replace with `ConcurrentHashMap.newKeySet()`, a synchronized diagnostic set, or owner-local/rate-limited recursion logging. |
...
| Must audit | JIT owner/static/runtime interaction | `jit-implications.md`, `scoped-value.md` | JIT has its own generated statics, `Ctx.Current`, classloader scope, and bridge module assumptions. | Do not change in this PR; prove classloader/container scope and document any JIT-specific owner tables before runtime integration. |
...
| Must audit, with must-fix subitems | Java `clone()` / `Cloneable` ownership paths | `Parameter.cloneBody()`, `MethodStructure.cloneBody()`, `ObjectHandle.cloneAs(...)`, `Constant.adoptedBy(...)`, AST/component clones | The focused audit found clone paths that can preserve original owners, share method/register state, shallow-copy runtime handles, or rely on opt-in constant-adoption validation. | Track and close the detailed findings in `clone-usage-audit.md`: replace object clones with explicit owner-aware copy/adoption APIs, and keep shallow array clones limited to documented container-copy use. |
