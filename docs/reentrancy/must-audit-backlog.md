...
| Done in this branch; scoped bridge remains | Public `ConstantPool.getCurrentPool()` compatibility API | `ambient-context-audit.md`, `scoped-value.md` | Keeping the getter public made new semantic misuse easy even after current call sites were fixed. | `getCurrentPool()` is private and guarded by `ConstantPoolDiagnosticsTest.currentPoolLookupIsPrivateBridgeOnly()`. Eventually replace `withPool(...)` with explicit owner APIs or a tightly scoped boundary object. |
