...
| Dimension lookahead | `ParserRecoveryTest` uses a non-deduplicating receiver to verify one cursor diagnostic when array types rewind their size expressions. `XdkArrayDimensionTest` checks FIRST_ERROR, cancellation and unchanged normal diagnostics. The lookahead listener branch is discarded on rewind; consumed syntax/failures merge it. |
