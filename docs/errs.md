...
component, parameters or method context. Metadata is final; its cursor child uses ordinary AST
ownership and cloning. Diagnostics remain until the source is repaired. Shared X91–X92 cover type
edits, incomplete outlines/folds and repair; IntelliJ labels its missing outline/fold checks partial.
See [C20/L35](errs-integration-plan.md#unfinished-declaration-headers) for boundaries and verification.
Validation passes 477 executed compiler tests, 1,019 executed LSP tests, 43 stdio cases and all
97 VS Code cases. IntelliJ passes startup plus 18 full and 15 partial scenarios, with no IDE
failures. The 120-cycle retention workload releases all 2,441 observed compiler objects.
...
Preparation stops before forwarding constructors, capture analysis and emission; Kotlin hides
generated class names in help. See [C18/L31](errs-integration-plan.md#anonymous-constructor-cursor-support)
for ownership, limits and validation. Manual/automated cases are X88–X89. All 473 executed Java,
986 executed LSP, 38 packaged-stdio and 94 editor cases pass; existing skips and retry evidence are
recorded there. The retention workload releases all 2,421 observed compiler objects.
...
addition is immutable `CursorBinding.argumentValues`: existing constructors remain, while record
patterns grow to eight components. There are no new AST fields or clone/reset obligations. Typed
prefix filtering, literal synthesis and implicit-property values remain follow-ups. See
[C13/L25, ownership, limits and validation](errs-integration-plan.md#argument-value-completion).
...
the host source index. Delegation and Ref/Var annotation dispatch remain separate. L50 adds ordinary source property-family rename.
...
| `Parser` function/sequence header recovery (C27) | Function returns use `Parameter` children and sequence arguments use their own delimiters; the parser owns finding the written leaf and respecting those boundaries. | Existing explicit-header flag and original leaf node only. No AST field, cached resolver, new context or clone/reset rule. Partial queries preserve ordinary missing-closer diagnostics and do not invent signatures. |
...
The [local extraction record](errs-integration-plan.md#fourth-local-extraction-batch-c3-2026-09-24)
now includes I3's required compiler consumer, C1's listener contract, C2's explicit propagation
