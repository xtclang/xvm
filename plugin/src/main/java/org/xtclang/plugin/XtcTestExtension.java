 ...
 * in the Gradle built-in plugins. If you want a project to use the default test task
 *  semantics and test a specific sequence of modules, this is where they are specified.
 * <p>
 * As with every other extension, by convention, it should be possible to change, extend
 * or override parts of it on individual test task level, so you should be able to create
 * a specialized test task with, e.g. verbose mode on, even though it's not on for
 * the entire project. You should also be able to create a specialized test task which
 * alters the sequence of modules to test, tests them in parallel with special logic
 ...
 * them.
 */
public interface XtcTestExtension extends XtcLauncherTaskExtension {

    /**
     * Return the plugin internal representation of a module execution config. This
     * is a tuple with a mandatory module name, and a list of arguments to pass to the
     ...
     * @return XtcTestModule instance
     */
    XtcTestModule module(Action<@NotNull XtcTestModule> action);

    ListProperty<@NotNull XtcTestModule> getModules();

    void moduleName(String name);

    void moduleNames(String... modules);

    void setModules(List<XtcTestModule> modules);

    void setModules(XtcTestModule... modules);
