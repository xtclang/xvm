 ...
 */
class ExceptionCollector(AbortExecutionPredicate? abortExecution = Null) {
    /**
    ...
    Result result.get() {
        ...
        if (exception.is(Exception)) {
            if (exception.is(PreconditionFailed)) {
                return new Result(Skipped, exception, suppressed, skipped=1);
            }
            if (hasAbortedExecution(exception)) {
                return new Result(Aborted, exception, suppressed, errors=1);
            }
            return new Result(Failed, exception, suppressed, failures=1);
        }
    ...
    private Boolean hasAbortedExecution(Exception? e) {
        AbortExecutionPredicate? abortExecution = this.abortExecution;
        if (e.is(Exception)) {
            return abortExecution.is(AbortExecutionPredicate) ? abortExecution(e) : False;
        }
        return False;
    }
}
