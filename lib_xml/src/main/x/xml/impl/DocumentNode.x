class DocumentNode(Element root)
    ...
    construct(Document that) {
        EachPart: for (Part part : that.parts) {
            Node node = makeNode(part);
            assert:arg !node.is(XmlDecl) || EachPart.first as "The XMLDecl must occur at the start of the document";
    ...
    @Override
    @RO Instruction? xmlDecl.get() = child_.is(XmlDecl) ?: Null;
