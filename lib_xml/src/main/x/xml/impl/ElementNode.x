        ...
        void set(String? newValue, Boolean cacheOnly = False) {
            if (cacheOnly) {
                ...
                if (newValue != oldValue) {
                    if (oldValue != Null) {
                        ...
                        if (contentCount > 0) {
                            ...
                            for (Int i : 0..<attributeCount) {
                                ...
                                prev = node;
                                node = node.as(Node).next_; // TODO CP get rid of .as()
                            }
                            ...
                            while (remaining > 0) {
                                if (node.is(ContentNode)) {
                                    ...
                                    if (!replaced && newValue != Null && remaining == contentCount
                                        ...
                                        Node? next = node.as(Node).next_;  // TODO CP get rid of .as()
                                        unlink_(prev, node);
                                        node = next;
                                    }
                                    --remaining;
                    ...
                    mod();
                    // TODO GG it didn't allow: super(newValue, cacheOnly=False);
            ...
            return new Iterator() {
                ...
                conditional Attribute next() {
                    if (Node prev := node.is(AttributeNode)) {
                        node = prev.as(Node).next_; // TODO CP get rid of as()
                        return True, prev;
                    }
                    return False;
                }
...
//            return cursor;
        ...
        AttributeList! clear() {
            ...
            while (node.is(AttributeNode)) {
                node.as(Node).parent_ = Null;   // TODO CP eliminate .as()
                Node prev = node;
                node = node.as(Node).next_;     // TODO CP eliminate .as()
                prev.as(Node).next_ = Null;     // TODO CP eliminate .as()
            }
        ...
        (Boolean found, Int index, AttributeNode!? prev, AttributeNode!? node) findNode(Int index) {
            ...
            for (Int i = 0; i < index; ++i) {
                prev = node;
                node = node.as(Node).next_.as(AttributeNode);     // TODO CP eliminate first .as()
            }
    ...
    protected (Node? prev, ElementNode? node) firstElement() {
        ...
        Node? node = prev?.as(Node).next_ : child_; // TODO CP get rid of .as()
        while (!node.is(ElementNode)) {
            prev = node;
            node = node?.next_ : assert; // we know there are ElementNodes in the list!
        }
        return prev, node.as(ElementNode); // TODO GG shouldn't need .as()
    }
