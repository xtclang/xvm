...
val listLocalPublications by tasks.registering {
    ...
    doLast {
        logger.lifecycle("'$name' Listing local publications (and their artifacts) for project '${project.group}:${project.name}':")
        val repoDir = File(System.getProperty("user.home"), ".m2/repository/org/xtclang/${project.name}")
        if (!repoDir.exists()) {
...
val listRemotePublications by tasks.registering {
    ...
    doLast {
        ...
        if (map.isEmpty()) {
            logger.warn("No packages found for project '${project.name}'.")
            return@doLast
        }
        logger.lifecycle("Listing all published packages for project '${project.name}':")
