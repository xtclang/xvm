...
abstract class DockerBuildTask @Inject constructor() : DefaultTask() {
    ...
    fun buildDockerImage() {
        ...
        val cmd = listOf("docker", "buildx", "build", "--platform", platformArg) +
                  ...
                  tags.get().flatMap { listOf("--tag", "${config.baseImage}:${it}") } +
                  listOf("--${action.get()}", ".")
...
class DockerPlugin : Plugin<Project> {
    override fun apply(project: Project) {
        // Plugin doesn't create tasks automatically - they're created by the build script
        // This just makes DockerBuildTask available
    }
}