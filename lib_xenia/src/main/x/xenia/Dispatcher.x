    ...
    void dispatch(RequestInfo requestInfo) {
        ...
        FromTheBeginning: while (True) {
            ...
            ProcessRequest: if (serviceInfo == Null) {
                ...
                // about the Session that it is associated with, so provide the Request with a
                // lambda to obtain the Session that we will discover after creating the Request
                @Volatile Session? session      = Null;
                @Volatile Boolean  sessionReady = False;
                function Session?() getSession = () -> {
                    // this assertion exists in case a Broker implementor attempts to obtain the
                    // Session from the Request provided to that Broker, when in reality the Broker
                    // is the only one to know how to obtain the Session
                    assert sessionReady as "Session is not yet obtained";
                    return session;
                };
                RequestIn    request = new Http1Request(requestInfo, sessionBroker, getSession,
                                                        endpoint.template, uriParams);
