...
public abstract class nLongRange
        extends Range {

    protected nLongRange(Ctx ctx, long first, long last, boolean firstExclusive, boolean lastExclusive) {
        super(ctx);
    ...
    /**
     * @return  the first element of the range packed into a Java long
     */
    protected abstract long $firstAsLong(Ctx ctx);

    /**
     * @return  the last element of the range packed into a Java long
     */
    protected abstract long $lastAsLong(Ctx ctx);
    ...
    public boolean $rangeTo128(Ctx ctx) {
        return $rangeTo128(ctx, $firstAsLong(ctx), $lastAsLong(ctx), firstExclusive$get$p(ctx), lastExclusive$get$p(ctx));
    }
