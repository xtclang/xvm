...
//            }
    ...
    void testProcess(Map<Int, Int> map)
        ...
        map.process^(0, e ->
            ...
            @Inject Timer timer;
            @Future Int   result;
            timer.schedule(Duration:0.1s, () ->
                {
                result = 42;
                });

            console.println($"3) process(0) {result}");
            e.value++;
            return result;
            });
