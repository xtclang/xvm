/**
 * This class will discover and execute `@Test` annotated methods in
 * a specific `Module`.
 */
const TestRunner(Module m) {
    ...
    void run() {
        console.print($"Running tests for module {m}");
        Class<Package> clz  = &m.class.as(Class<Package>);
        (Int passed, Int failed) = runTests(clz);
        console.print($"Tests completed for module {m}");
    ...
    (Int, Int) runTests(Class clz) {
        ...
        for (Type childType : clz.toType().childTypes.values) {
            if (Class childClass := childType.fromClass()) {
                ...
                if (childClass.path.startsWith(m.qualifiedName)) {
    ...
    (Int, Int) executeTest(Class clz, Test[] testMethods) {
        ...
        for (Test test : testMethods) {
            try {
                provider.setTest(clz.name, test);
                if (test.is(Method)) {
