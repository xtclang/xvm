 ...
 * significant byte of the integer, as is. When reading in a packed integer, if the leftmost bits of
 * the first byte are <b>not</b> {@code 10}, then the XIP'd integer is in small format, and simply
 * requires sign extension of the first byte to the desired integer size in order to form the
 * integer value.
 * </li><li>
 * <b>Medium</b>: For a value in the range {@code -4096..4095} (13 bits), the value is encoded in
 * two bytes. The first byte contains the binary value {@code 100} in the most significant 3 bits,
 * indicating the medium format. The 2's complement integer value is formed from the least
 * significant 5 bits of the first byte, sign extended to the desired integer size and then
 * left-shifted 8 bits, or'd with the bits of the second byte.
 * </li><li>
 * <b>Large</b>: For any 2's complement integer value from {@code 2..32} bytes (in the range
 * {@code -(2^255)..2^255-1}), let {@code b} be that number of bytes, with the XIP'd value encoded
 * in {@code 1+b} bytes. The first byte contains the binary value {@code 101} in the most
 * significant 3 bits, and the remaining 5 bits are the unsigned value {@code b-1}, indicating the
 * large format. (Note: Since {@code b} is at least {@code 2}, the value {@code b-1} is always
 * non-zero.) The following {@code b} bytes form the 2's complement integer value.
 * </li><li>
 * <b>Huge</b>: For any integer value {@code n} larger than 32 bytes, let {@code b} be that number
 * of bytes. The first byte of the XIP'd integer is {@code 0xA0}, which indicates the huge format.
...
public final class PackedInteger
    ...
    /**
     * Construct a PackedInteger by reading the packed value from a DataInput stream.
     *
    ...
    /**
     * The size of the "native" 2's-complement signed integer that would be necessary to hold the
     * value.
     *
    ...
    /**
     * Shift left the bits in the value of this PackedInteger by the specified count, resulting in a
     * new PackedInteger.
     *
    ...
    /**
     * Format the PackedInteger as a String of the specified radix, including a radix prefix for any
     * non-decimal radix.
     *
    ...
    /**
     * Verify that the PackedInteger is still uninitialized (has no integer value).
     *
     * @throws IllegalStateException if the value of the PackedInteger has already been set
     */
    ...
    /**
     * Verify that the PackedInteger has been successfully initialized to an integer value.
     *
     * @throws IllegalStateException if the value of the PackedInteger has <b>not</b> yet been set
     */
     ...
     *
     * @return the minimum number of bytes to hold the integer value in 2s-complement form
     *
    ...
    /**
     * Smallest integer value to cache. One quarter of the cache size is reserved for small negative
     * integer values.
     */
    private static final long CACHE_MIN = -(CACHE.length >> 2);

    /**
     * Largest integer value to cache. Three-quarters of the cache size is reserved for small
