package org.xvm.javajit.intrinsic.collections;

import org.xvm.javajit.Ctx;
import org.xvm.javajit.intrinsic.xException;
import org.xvm.javajit.intrinsic.xObj;
import org.xvm.javajit.intrinsic.xRangeInt64;
 ...
 */
public abstract class xArray<Element extends xObj>
        extends xObj {

    public xArray(long containerId) {
        super(containerId);
    }
...
*/
     ...
     */
    protected abstract xArray<Element> $especial();

    /**
     * @return one of {@link #MUTABLE}, {@link #FIXED}, {@link #PERSISTENT}, or {@link #CONSTANT}
     */
    public int $mut() {
        xArray<Element> especial = $especial();
...
//    }
     ...
     */
    public xArray<Element> $slice(Ctx ctx, long n) {
//        if ($empty(n)) {
//            return TODO
//        }
        return $slice(ctx, xRangeInt64.$first(n), xRangeInt64.$size(n), xRangeInt64.$descending(n));
    }
     ...
     * @return the specified array slice
     */
    public xArray<Element> $slice(Ctx ctx, long n1, long n2) {
//        if ($empty(n)) {
//            return TODO
//        }
        return $slice(ctx, xRangeInt64.$first(n1, n2), xRangeInt64.$size(n1, n2), xRangeInt64.$descending(n1, n2));
    }

    public abstract xArray<Element> $slice(Ctx ctx, long offset, long size, boolean descending);
...
//    public abstract Element $get(long slice, long index);
     ...
     * @throws xException
     */
    protected xException $oob(long index) {
        if (index < 0) {
            throw xException.$oob("negative index: " + index, null);
        }
        throw xException.$oob("index: " + index + " (size=" + $size() + ")", null);
    }
     ...
     * @throws xException
     */
    protected xException $ro() {
        throw xException.$ro("array mutability=" + $mutDesc(), null);
    }
