...
public class TypeSystem {
    ...
    TypeSystem(Xvm xvm, ModuleRepository repo) {
        ...
        ArrayList<ModuleStructure> list = new ArrayList<>(fs.getChildrenCount());
        for (Iterator iter = fs.ensureChildByNameMap().values().iterator(); iter.hasNext(); ) {
            if (iter.next() instanceof ModuleStructure module) {
                list.add(module);
            }
        }
    ...
    public Linker create() {
        ...
        for (ModuleLoader loader : loader.owned) {
        ...
        return new Linker(xvm).addSharedModule(ecstasyLoader);
    }
