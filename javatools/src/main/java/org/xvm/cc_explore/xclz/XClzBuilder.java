...
public class XClzBuilder {
  ...
  private static final SB ASB = new SB().ii();
  ...
  private SB value_tcon( TCon tc, boolean nested ) {
    ...
    if( tc instanceof AryCon ac ) {
      ...
      final int n = _fcons._len;  // JCON#
      // Top level needs the initial variable declaration
      if( !nested )
        ASB.p("private static final ").p(type).p(" JCON").p(n).p(" = ");
      ASB.p("new ").p(type).p("() {{").nl().ii();
      if( ac.cons()!=null ) {
        for( Const con : ac.cons() ) {
          ASB.ip("add(");
          value_tcon( (TCon)con, true );
          if( con instanceof AryCon )                    // Multiline?
            ASB.unchar(System.lineSeparator().length()); // Remove newline
          ASB.p(");").nl();
        }
      }
      ASB.di().ip("}}");
      // Top level record for later printing
      if( nested ) {
        ASB.nl();
      } else {
        ASB.p(";");
        _fcons.push(ASB.toString());
        ASB.clear().p("JCON").p(n); // Clear and return the var name
      }
      return ASB;
    }
    // Booleans
