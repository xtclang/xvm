...
public class xRTType
    ...
    public static xRTType getInstance(Frame frame) {
        return NativeTemplates.get(frame).type();
    }

    public static xRTType getInstance(Container container) {
        return NativeTemplates.get(container).type();
    }
    ...
    public int createConstHandle(Frame frame, Constant constant) {
        if (constant instanceof TypeConstant typeTarget) {
            ...
            return frame.pushStack(typeData.normalizeParameters().ensureTypeHandle(frame.container()));
        }
    ...
    public int createProxyHandle(Frame frame, ServiceContext ctxTarget, ObjectHandle hTarget,
        ...
        return frame.assignValue(Op.A_STACK,
                makeForeignHandle(frame.container(), ((TypeHandle) hTarget).getUnsafeDataType()));
    }
    ...
    public int getPropertyValue(Frame frame, ObjectHandle hTarget, PropertyConstant idProp, int iReturn) {
        ...
        if (idProp.isFormalType()) {
            ...
            return typeValue == null
                ...
                : frame.assignValue(iReturn, typeValue.ensureTypeHandle(frame.container()));
        }
    ...
    public int invokeNativeN(Frame frame, MethodStructure method, ObjectHandle hTarget,
        ...
        case "dump":
            ...
            return frame.assignValue(iReturn,
                xString.makeHandle(frame, hType.getUnsafeDataType().ensureTypeInfo(frame.container().getErrorListener()).toString(false)));
        }
    ...
    private int ensureImmutable(Frame frame, TypeHandle hType, int iReturn) {
        ...
        return type.isImmutabilitySpecified()
                ...
                : frame.assignValue(iReturn,
                    type.freeze().ensureTypeHandle(frame.container()));
    }
...
//        }
    ...
    public int extractArrayValue(Frame frame, ObjectHandle hTarget, long lIndex, int iReturn) {
        ...
        // range-checked before narrowing; see the note in xString.extractArrayValue
        return lIndex >= 0 && lIndex < type.getParamsCount()
            ? frame.assignValue(iReturn,
                    type.getParamType((int) lIndex).ensureTypeHandle(frame.container()))
    ...
    public int getPropertyChildTypes(Frame frame, TypeHandle hType, int iReturn) {
        ...
        return Utils.constructListMap(frame, clzListMap,
                xArray.makeStringArrayHandle(container, ahName),
    ...
    private int getForeignChildTypes(Frame frame, TypeHandle hType, int iReturn) {
        ...
        return Utils.constructListMap(frame, container.resolveClass(ensureListMapType(container)),
                xString.ensureEmptyArray(container), ensureEmptyTypeArray(container), iReturn);
    }
    ...
    public int getPropertyConstructors(Frame frame, TypeHandle hType, int iReturn) {
        ...
        if (infoTarget.isNewable(false, ErrorListener.BLACKHOLE)) {
            ...
            for (MethodConstant idConstr :
                ...
                TypeConstant[]  atypeParams = infoMethod.getSignature().getRawParams().unsafeArray();
    ...
    public int getPropertyExplicitlyImmutable(Frame frame, TypeHandle hType, int iReturn) {
        return frame.assignValue(iReturn,
                xBoolean.makeHandle(frame, hType.getDataType().isImmutabilitySpecified()));
    }
    ...
    public int getPropertyForm(Frame frame, TypeHandle hType, int iReturn) {
        return frame.assignDeferredValue(iReturn, ensureFormHandle(frame, hType.getUnsafeDataType()));
    }
    ...
    public int getPropertyFunctions(Frame frame, TypeHandle hType, int iReturn) {
        ...
        TypeConstant                    typeTarget  = hType.getDataType();
        Map<MethodConstant, MethodInfo> mapMethods  = typeTarget.ensureTypeInfo(frame.container().getErrorListener()).getMethods();
    ...
    public int getPropertyMethods(Frame frame, TypeHandle hType, int iReturn) {
        ...
        TypeConstant            typeTarget  = hType.getDataType();
        var                     aMethods    = typeTarget.ensureTypeInfo(frame.container().getErrorListener()).sortedMethods();
        ...
        if (containerForeign != null) {
            Op opCall = new Op() {
                public int process(Frame frame, int iPC) {
                    ...
                    case Op.R_NEXT:
                        return createProxyArray(frame, (ArrayHandle) frame.popStack(),
                                xRTMethod.getInstance(container).getCanonicalClass());
    ...
    public int getPropertyProperties(Frame frame, TypeHandle hType, int iReturn) {
        ...
        TypeConstant            typeTarget = hType.getDataType();
        var                     aProps     = typeTarget.ensureTypeInfo(frame.container().getErrorListener()).sortedProperties();
    ...
    public int getPropertyRecursive(Frame frame, TypeHandle hType, int iReturn) {
        return frame.assignValue(iReturn, xBoolean.makeHandle(frame, hType.getDataType().containsRecursiveType()));
    }
    ...
    public int getPropertyTemplate(Frame frame, TypeHandle hType, int iReturn) {
        return frame.assignValue(iReturn,
                xRTTypeTemplate.makeHandle(frame.container(), hType.getDataType()));
    }
    ...
    public int getPropertyTypeSystem(Frame frame, TypeHandle hType, int iReturn) {
        ...
        if (hType.isForeign()) {
            ...
            return frame.container().ensureTypeSystemHandle(frame, iReturn);
        }
    }
    ...
    protected int invokeAnnotate(Frame frame, TypeHandle hType, ObjectHandle hArg, int iReturn) {
        ...
        if (hMixin instanceof ClassHandle hClass) {
            ...
            if (xArray.getInstance(frame).size(hArgs) > 0) {
                // TODO args
        ...
        return frame.raiseException(xException.invalidType(frame, "Type \"" +
                ...
                frame.container().getModule().getName() + '"'));
    }
    ...
    private int makeAnnotated(Frame frame, TypeConstant typeThis, TypeConstant typeAnno, int iReturn) {
        ...
        return frame.assignValue(iReturn, typeResult.ensureTypeHandle(frame.container()));
    }
    ...
    protected int invokeContained(Frame frame, TypeHandle hType, int[] aiReturn) {
        ...
        if (typeTarget.isVirtualChild() || typeTarget.isAnonymousClass()) {
            TypeHandle hParent = typeTarget.getParentType().ensureTypeHandle(frame.container());
            ...
            if (type.isSingleDefiningConstant()) {
                ...
                if (constDef instanceof PropertyConstant idProp) {
                    ...
                    PropertyInfo infoProp   = frame.poolContext().ensureAccessTypeConstant(
                            typeParent, Access.PRIVATE).ensureTypeInfo(frame.container().getErrorListener()).findProperty(idProp, true);
                    ...
                    if (infoProp != null) {
                        ...
                        return Op.isDeferred(hProp)
                            ...
                            : frame.assignValues(aiReturn, xBoolean.trueHandle(frame), hProp);
                    }
                }
            }
        }
    ...
    protected int invokeIsNullable(Frame frame, TypeHandle hType, int[] aiReturn) {
        ...
        return type.isNullable()
                ? frame.assignValues(aiReturn, xBoolean.trueHandle(frame),
                        type.removeNullable().ensureTypeHandle(frame.container()))
    ...
    protected int invokeModifying(Frame frame, TypeHandle hType, int[] aiReturn) {
        ...
        return type.isModifyingType()
                ? frame.assignValues(aiReturn, xBoolean.trueHandle(frame),
                        type.getUnderlyingType().ensureTypeHandle(frame.container()))
                : frame.assignValue(aiReturn[0], xBoolean.falseHandle(frame));
    }
    ...
    protected int invokeParameterized(Frame frame, TypeHandle hType, int[] aiReturn) {
        ...
        for (int i = 0; i < cTypes; ++i) {
            ahTypes[i] = atypes[i].ensureTypeHandle(frame.container());
        }

        ObjectHandle hArray = xArray.createImmutableArray(
                                ensureTypeArrayComposition(frame.container()), ahTypes);
    ...
    public int invokeResolveFormalType(Frame frame, TypeHandle hType, StringHandle hName, int[] aiReturn) {
        ...
        return typeR == null
            ? frame.assignValue(aiReturn[0], xBoolean.falseHandle(frame))
            : frame.assignValues(aiReturn, xBoolean.trueHandle(frame), typeR.ensureTypeHandle(frame.container()));
    }
    ...
    public int invokeStructConstructor(Frame frame, TypeHandle hType, ObjectHandle hOuter, int[] aiReturn) {
        ...
        return frame.assignValues(aiReturn, xBoolean.trueHandle(frame), hConstructor);
    }
     ...
     *
     * @return the initialized handle to the appropriate Ecstasy {@code Type.Access} enum value
     *
     ...
     */
    public static ObjectHandle ensureAccessHandle(Frame frame, Access access) {
        xEnum enumAccess = frame.container().getEnumTemplate("reflect.Access");
        return switch (access) {
            case PUBLIC    -> enumAccess.ensureEnumByName(frame, "Public");
            case PROTECTED -> enumAccess.ensureEnumByName(frame, "Protected");
            case PRIVATE   -> enumAccess.ensureEnumByName(frame, "Private");
            case STRUCT    -> enumAccess.ensureEnumByName(frame, "Struct");
        };
    }
     ...
     *
     * @return the initialized handle to the appropriate Ecstasy {@code Type.Form} enum value
     *
     * Note: this returns {@link ObjectHandle}, not {@code EnumHandle}, because natural enum lookup
     * can initially produce a construction struct. The public publication boundary must resolve
     * through the enum's SingletonConstant, and that resolution can be deferred.
     */
    protected static ObjectHandle ensureFormHandle(Frame frame, TypeConstant type) {
        xEnum enumForm = frame.container().getEnumTemplate("reflect.Type.Form");
        ...
        case TerminalType:
            ...
            if (type.isSingleDefiningConstant()) {
                return switch (type.getDefiningConstant().getFormat()) {
                    case NativeClass ->
                        enumForm.ensureEnumByName(frame, "Pure");

                    case Module, Package, Class, ThisClass, ParentClass, ChildClass,
                         IsClass, IsConst, IsEnum, IsModule, IsPackage ->
                        enumForm.ensureEnumByName(frame, "Class");

                    case Property ->
                        enumForm.ensureEnumByName(frame, "FormalProperty");

                    case TypeParameter ->
                        enumForm.ensureEnumByName(frame, "FormalParameter");

                    case FormalTypeChild ->
                        enumForm.ensureEnumByName(frame, "FormalChild");
        ...
        case ParameterizedType:
            ...
            return ensureFormHandle(frame, type.getUnderlyingType());
    ...
    private int createProxyArray(Frame frame, ArrayHandle hArray, TypeComposition clzElement) {
        ...
        TypeComposition clzArray    = frame.container().resolveClass(
        ...
        for (int i = 0; i < cValues; i++) {
            ...
            case Op.R_NEXT:
                ...
                ahValue[i] = hElement instanceof TypeHandle hType
                        ? xRTType.makeForeignHandle(frame.container(), hType.getUnsafeType())
