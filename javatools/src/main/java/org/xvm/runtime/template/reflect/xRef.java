...
public class xRef
    ...
    protected xRef(Container container, ClassStructure structure, NativeRole role) {
        ...
        f_constInception = canonical
                ...
                : null;
        f_sigGet = canonical
                ? Lazy.ofBound(xRef::resolveGetSignature)
                : Lazy.ofBound(owner -> xRef.getInstance(owner.container()).getGetSignature());
    }
    ...
    public int invokeNativeNN(Frame frame, MethodStructure method,
        ...
        case "revealStruct":
            return actOnReferent(frame, hRef, h -> revealStruct(frame, h, (TypeHandle) ahArg[0], aiReturn));
        }
    ...
    private final Lazy.Bound<xRef, SignatureConstant> f_sigGet;
