...
public class xNanosTimer
        ...
        public class Alarm {
            ...
            public void cancel() {
                synchronized (this) {
                    ...
                    cancelTrigger();
                    // m_fDead guarantees run() has not fired and never will, so the callback
                    // entry would otherwise leak in the service registry
     ...
     */
    private final Lazy.Bound<xNanosTimer, TypeComposition> f_clzDuration =
            Lazy.ofBound(owner -> owner.container().getTemplate("temporal.Duration").getCanonicalClass());
