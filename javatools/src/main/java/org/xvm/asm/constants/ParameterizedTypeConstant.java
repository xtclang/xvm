...
public final class ParameterizedTypeConstant
    ...
    public ResolutionResult resolveContributedName(
        ...
        ResolutionResult result = super.resolveContributedName(sName, access, idMethod, collector, errs);
        if (result == ResolutionResult.RESOLVED) {
            return result;
        }

        for (TypeConstant typeParam : atypeParams()) {
            ResolutionResult resultParam = typeParam.resolveContributedName(sName, access, idMethod, collector, errs);
    ...
    public String getValueString() {
        ...
        // The old special case printed function types as the prettier "function R(P)" via
        // m_constType.isA(pool.typeFunction()) - which interns the canonical Function type and mutates
        // relation caches, so merely rendering a type mutated the pool. Dropped: functions now render
        // structurally, consistent with the runtime (reflect/Type.x already renders Function<...>). See
