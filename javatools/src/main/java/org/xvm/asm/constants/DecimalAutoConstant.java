...
public class DecimalAutoConstant
    ...
    public Constant apply(Token.Id op, Constant that) {
        ...
        case "Dec+Dec":
            return getConstantPool().ensureDecAConstant(this.getValue().add(((DecimalAutoConstant) that).getValue()));

        case "Dec-Dec":
            return getConstantPool().ensureDecAConstant(this.getValue().subtract(((DecimalAutoConstant) that).getValue()));

        case "Dec*Dec":
            return getConstantPool().ensureDecAConstant(this.getValue().multiply(((DecimalAutoConstant) that).getValue()));

        case "Dec/Dec":
            return getConstantPool().ensureDecAConstant(this.getValue().divide(((DecimalAutoConstant) that).getValue()));

        case "Dec%Dec":
            return getConstantPool().ensureDecAConstant(this.getValue().mod(((DecimalAutoConstant) that).getValue()));

        case "Dec^Dec":
            return getConstantPool().ensureDecAConstant(this.getValue().pow(((DecimalAutoConstant) that).getValue()));
        ...
        case "Dec>=Dec":
        case "Dec<==>Dec":
            return translateOrder(this.getValue().compareForObjectOrder(((DecimalAutoConstant) that).getValue()), op);
        }
