...
public sealed class IntersectionTypeConstant
    ...
    @Override
    protected IntersectionTypeConstant copyForAdoption(AdoptionContext context) {
        var pool = requireSharedAdoptionPool(context, "intersection type with foreign child type");
        ...
        return new IntersectionTypeConstant(pool, m_constType1, m_constType2);
    }
    ...
    protected Map<Nid, ParamInfo> mergeTypeParams(TypeInfo info1, TypeInfo info2, ErrorListener errs) {
        ...
        for (Map.Entry<Nid, ParamInfo> entry : map.entrySet()) {
        ...
        for (Map.Entry<Nid, ParamInfo> entry : map2.entrySet()) {
            Nid nid = entry.getKey();
