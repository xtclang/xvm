...
public class MethodInfo
    ...
    /**
     * If this MethodInfo represents a virtual call chain that spans multiple virtual child classes,
     * find and adopt any missing virtual methods.
     * <p/>
     * Consider the following class layout (skipping Override annotations for simplicity):
     *
     * <pre><code>
     *    class BaseParent
     *     {
     ...
     * </code></pre>
     *
     *  When we merge the method body for DP.DC.foo(), the base chain is:
     *  <pre>
     *       BP.DC -> BP.BC
     *  </pre>
     *  and the logic above produces the following chain:
     *  <pre>
     *       DP.DC -> BP.DC -> BP.BC
     *  </pre>
     *  However, we need to produce a chain that encompasses virtual child methods as well:
     *  <pre>
     *       DP.DC -> BP.DC -> DP.BC -> BP.BC
     *  </pre>
     *
     * @param typeChild  the virtual child type
     * @param clzChild   the virtual child class structure
     * @param errs       the error list to log any errors to
     *
     * @return a MethodInfo to use in place of this
     */
    public MethodInfo adoptVirtual(TypeConstant typeChild, ClassStructure clzChild, ErrorListener errs)
        {
        assert typeChild.isVirtualChild() && clzChild.isVirtualChild();

        ClassStructure clzSuper = clzChild.getSuper();
        if (clzSuper == null)
            {
            return this;
            }
        ...
        if (cBodies > 1)
            {
            MethodBody bodyTop = aBody[0];
            if (!bodyTop.getIdentity().isTopLevel())
                {
                return this;
                }

            String                sNameDC = clzChild.getName();
            ArrayList<MethodBody> listTop = new ArrayList<>();
            listTop.add(bodyTop);

            for (int i = 1; i < cBodies; i++)
                {
                MethodBody       bodyNext = aBody[i];
                IdentityConstant idNext   = bodyNext.getIdentity().getNamespace();

                if (!idNext.isClass() ||
                        !((ClassStructure) idNext.getComponent()).isVirtualChild())
                    {
                    return this;
                    }

                String sNameBC = idNext.getName();
                if (sNameBC.equals(sNameDC))
                    {
                    // the same child name; add the method and continue
                    listTop.add(bodyNext);
                    continue;
                    }

                // we found a transition from DC to a different child name; however, we should not
                // use it, but instead find a very first super class of a different name, since
                // there is a possibility that it is extended by another virtual child (DP.BC) that
                // implements this method
                String sNextName;
                while (true)
                    {
                    sNextName = clzSuper.getName();

                    if (!sNextName.equals(sNameDC))
                        {
                        break;
                        }
                    clzSuper = clzSuper.getSuper();
                    }

                ConstantPool pool      = pool();
                TypeConstant typeDP    = typeChild.getParentType();
                TypeInfo     infoDP    = typeDP.ensureTypeInfoInternal(errs);
                TypeConstant typeBP_BC = clzSuper.getFormalType().resolveGenerics(pool, typeChild);
                TypeConstant typeDP_BC = infoDP.calculateChildType(pool, sNextName).
                                            adoptParameters(pool, typeBP_BC);
                TypeInfo     infoDP_BC = pool.ensureAccessTypeConstant(typeDP_BC, Access.PROTECTED).
                                            ensureTypeInfoInternal(errs);
                MethodInfo methodNext  = infoDP_BC.getMethodById(bodyNext.getIdentity());
                assert methodNext != null;

                if (methodNext.isCapped())
                    {
                    methodNext = infoDP_BC.getNarrowingMethod(methodNext);
                    }

                // replace the tail of our chain with the newly calculated virtual one
                MethodBody[] abodyTail = methodNext.getChain();
                appendList(listTop, abodyTail, 0, abodyTail.length);
                return new MethodInfo(listTop.toArray(MethodBody.NO_BODIES));
                }
            }
