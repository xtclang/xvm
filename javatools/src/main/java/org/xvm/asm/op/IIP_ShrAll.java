...
/**
 * IIP_SHRALL rvalue-target, rvalue-ix, rvalue2 ; T[ix] %= T
 */
public class IIP_ShrAll
        extends OpIndexInPlace
    {
    /**
     * Construct an IIP_SHRALL op for the passed target.
     *
     * @param argTarget  the target Argument
     * @param argIndex   the index Argument
     * @param argValue   the value Argument
     */
    public IIP_ShrAll(Argument argTarget, Argument argIndex, Argument argValue)
        {
     ...
     */
    public IIP_ShrAll(DataInput in, Constant[] aconst)
            throws IOException
        {
        super(in, aconst);
        }
    ...
    protected int complete(Frame frame, ObjectHandle hTarget, JavaLong hIndex, ObjectHandle hValue)
        ...
        switch (hCurrent.getOpSupport().invokeShrAll(frame, hCurrent, hValue, A_STACK))
            {
            case R_NEXT:
