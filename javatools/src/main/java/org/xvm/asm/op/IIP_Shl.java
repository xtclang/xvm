...
/**
 * IIP_SHL rvalue-target, rvalue-ix, rvalue2 ; T[ix] <<= T
 */
public class IIP_Shl
        extends OpIndexInPlace
    {
    /**
     * Construct an IIP_SHL op for the passed target.
     *
     * @param argTarget  the target Argument
     * @param argIndex   the index Argument
     * @param argValue   the value Argument
     */
    public IIP_Shl(Argument argTarget, Argument argIndex, Argument argValue)
        {
     ...
     */
    public IIP_Shl(DataInput in, Constant[] aconst)
            throws IOException
        {
        super(in, aconst);
        }
    ...
    protected int complete(Frame frame, ObjectHandle hTarget, JavaLong hIndex, ObjectHandle hValue)
        ...
        switch (hCurrent.getOpSupport().invokeShl(frame, hCurrent, hValue, A_STACK))
            {
            case R_NEXT:
