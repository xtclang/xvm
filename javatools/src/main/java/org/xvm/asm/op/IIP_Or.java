...
/**
 * IIP_OR rvalue-target, rvalue-ix, rvalue2 ; T[ix] |= T
 */
public class IIP_Or
        extends OpIndexInPlace
    {
    /**
     * Construct an IIP_Or op for the passed target.
     *
     * @param argTarget  the target Argument
     * @param argIndex   the index Argument
     * @param argValue   the value Argument
     */
    public IIP_Or(Argument argTarget, Argument argIndex, Argument argValue)
        {
     ...
     */
    public IIP_Or(DataInput in, Constant[] aconst)
            throws IOException
        {
        super(in, aconst);
        }
    ...
    protected int complete(Frame frame, ObjectHandle hTarget, JavaLong hIndex, ObjectHandle hValue)
        ...
        switch (hCurrent.getOpSupport().invokeOr(frame, hCurrent, hValue, A_STACK))
            {
            case R_NEXT:
