...
public class VersionTree<V>
    ...
    public Iterator<Version> iterator() {
        return new Iterator<>() {
            ...
            private Node<V> loadNext() {
                ...
                List<Node<V>> kids = prev.kids;
                if (kids != null && !kids.isEmpty()) {
                    return next = kids.get(0).firstContainedPresent();
                }
    ...
    public Version findLowestVersion() {
        return isEmpty()
                ? null
                : root.kids.get(0).firstContainedPresent().getVersion();
    }
    ...
    private static class Node<V> {
        ...
        Node<V> firstContainedPresent() {
            ...
            return kids.get(0).firstContainedPresent();
        }
        ...
        Node<V> nextSibling() {
            ...
            List<Node<V>> siblings = parent.kids;
            for (int i = 0, c = siblings.size(); i < c; ++i) {
                if (siblings.get(i) == this) {
                    return i < c-1 ? siblings.get(i+1) : null;
                }
            }
        ...
        void render(StringBuilder sb, String sIndentFirst, String sIndent) {
            ...
            if (kids != null) {
                ...
                for (int i = 0, c = kids.size(); i < c; ++i) {
                    Node<V> cur = kids.get(i);
        ...
        /**
         * @param nodes  the children, or null
         * @param node   the child to look for
         *
         * @return the index of the child, or -1 if absent
         */
        private static <V> int indexOf(List<Node<V>> nodes, Node<V> node) {
            return nodes == null ? -1 : nodes.indexOf(node);
        }

        /**
         * @param nodes  the children, or null
         * @param part   the version part to look for
         *
         * @return the index of the child with that part, or -1 if absent
         */
        private static <V> int indexOf(List<Node<V>> nodes, int part) {
            if (nodes != null) {
                for (int i = 0, c = nodes.size(); i < c; ++i) {
                    if (nodes.get(i).part == part) {
                        return i;
                    }
                }
            }
        ...
        /**
         * Insert a child, keeping the children ordered ascending by version part.
         *
         * @param node  the child to add
         */
        private void addNode(Node<V> node) {
            List<Node<V>> nodes = kids;
            if (nodes == null) {
                kids = nodes = new ArrayList<>(4);
            }

            int part = node.part;
            for (int i = 0, c = nodes.size(); i < c; ++i) {
                int partCur = nodes.get(i).part;
                assert part != partCur;
                if (part < partCur) {
                    nodes.add(i, node);
                    return;
