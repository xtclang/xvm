...
public abstract class OpOptimized
    ...
    /**
     * Invoke an optimized method that takes no arguments on an XVM primitive target.
     *
     * @param bctx         the current build context
     * @param code         the code builder to add the op codes to
     ...
     */
    protected JitMethodDesc buildOptimizedXvmPrimitive(BuildContext bctx, CodeBuilder code,
                                                       RegisterInfo regTarget, MethodInfo method) {
        return buildOptimizedXvmPrimitive(bctx, code, regTarget, method, NO_ARGS);
    }
