...
public class LiteralExpression
    ...
    public TypeConstant getImplicitType(Context ctx) {
        ...
        return switch (literal.getId()) {
            case LIT_BIT    -> pool.typeBit();
            case LIT_NIBBLE -> pool.typeNibble();
            case LIT_CHAR   -> pool.typeChar();
            case TODO,                  // the T0D0 keyword has a String text for the token's value
                 LIT_STRING -> pool.typeString();
            case LIT_BINSTR -> pool.typeBinary();
            case LIT_INT    -> pool.typeIntLiteral();
            case LIT_INT8   -> pool.typeInt8();
            case LIT_INT16  -> pool.typeInt16();
            case LIT_INT32  -> pool.typeInt32();
            case LIT_INT64  -> pool.typeInt64();
            case LIT_INT128 -> pool.typeInt128();
            case LIT_INTN   -> pool.typeIntN();
            case LIT_UINT8  -> pool.typeUInt8();
            case LIT_UINT16 -> pool.typeUInt16();
            case LIT_UINT32 -> pool.typeUInt32();
            case LIT_UINT64 -> pool.typeUInt64();
            case LIT_UINT128-> pool.typeUInt128();
            case LIT_UINTN  -> pool.typeUIntN();
            case LIT_DEC    -> pool.typeFPLiteral();
            case LIT_DECA   -> pool.typeDec64();
            case LIT_DEC32,
                 LIT_DEC64,
                 LIT_DEC128,
                 LIT_DECN   -> pool.ensureEcstasyTypeConstant("numbers." + literal.getId().TEXT);
            case LIT_FLOAT  -> pool.typeFPLiteral();
            case LIT_FLOAT8E4,
                 LIT_FLOAT8E5,
                 LIT_BFLOAT16,
                 LIT_FLOAT16,
                 ...
                 LIT_FLOAT64,
                 LIT_FLOAT128,
                 LIT_FLOATN    -> pool.ensureEcstasyTypeConstant("numbers." + literal.getId().TEXT);
            case LIT_DATE      -> pool.typeDate();
            case LIT_TIMEOFDAY -> pool.typeTimeOfDay();
            case LIT_TIME      -> pool.typeTime();
            case LIT_TIMEZONE  -> pool.typeTimeZone();
            case LIT_DURATION  -> pool.typeDuration();
            case LIT_VERSION   -> pool.typeVersion();
            case LIT_PATH      -> pool.typePath();

            default -> throw new IllegalStateException(literal.getId().name() + "=" + literal.getValue());
        };
    }
    ...
    public String toString() {
        return switch (literal.getId()) {
            case LIT_INT,
                 LIT_DEC,
                 LIT_FLOAT     -> String.valueOf(literal.getValue());
            case LIT_BIT,
                 LIT_NIBBLE,
                 LIT_INT8,
                 LIT_INT16,
                 ...
                 LIT_FLOAT64,
                 LIT_FLOAT128,
                 LIT_FLOATN    -> literal.toString();
            case LIT_CHAR      -> Handy.quotedChar((Character) literal.getValue());
            case LIT_STRING    -> Handy.quotedString(String.valueOf(literal.getValue()));
            case LIT_BINSTR    -> '#' + Handy.byteArrayToHexString((byte[]) literal.getValue()).substring(2);
            case LIT_DATE      -> "Date:" + literal.getValue();
            case LIT_TIMEOFDAY -> "TimeOfDay:" + literal.getValue();
            case LIT_TIME      -> "Time:" + literal.getValue();
            case LIT_TIMEZONE  -> "TimeZone:" + literal.getValue();
            case LIT_DURATION  -> "Duration:" + literal.getValue();
            case LIT_VERSION   -> "v:" + literal.getValue();
            case LIT_PATH      -> "Path:" + literal.getValue();
            case TODO          -> "TODO(" + Handy.quotedString(String.valueOf(literal.getValue())) + ')';
