...
class CanaryCohortGcTest {
    ...
    static final long COH_SIZE = ShallowSizeOf.instanceOf(CohortRef.class);
...
//        });
        ...
        out("refSize=" + REF_SIZE + ", cohSize=" + COH_SIZE + ", objSize=" + OBJ_SIZE + ", listSize=" + LIST_SIZE);
...
//                    }
                ...
                if (size > available) {
                    out("FAIL: available=" + available + ", size=" + size + ", list=" + (list == null ? "null" : list.size() + " elements") + "; reclaims(buddy=" + buddyReclaims + ", canary=" + canaryReclaims + ")");
            ...
            try {
                while (queue.poll() instanceof CleanablePhantom<?> ref) {
                    ...
                    if (ref instanceof Reclaim<?> reclaim) {
                        ...
                        long size = reclaim.size();
                        release += size;
                        reclaim.adjustSize(-size);
...
//                out("reclaimed " + bytes + " bytes (" + before + "->" + available + ") from " + count + " objects in " + elapsed + "ms");
    ...
    public static final class Ctx {
        public Ctx(Container container) {
            ...
            this.canary.reclaim = new Reclaim<Canary>(canary, container.xvm.refQueue, container.id, 0);
        ...
        CleanablePhantom<?> firstWatch;
        CleanablePhantom<?> lastWatch;
         ...
         *
         * @param toWatch the new orphan (last allocated object)
        ...
        void watch(Collectable toWatch, long size) {
            if (RECLAIM) {
                ...
                if (canary.reclaim.size() >= CANARY_SIZE) {
                    // complete the cohort ring
                    firstWatch.prev = watch;
                    watch.next = firstWatch;

                    canary.ctx = null;
                    container.xvm.keepalive = canary.reclaim.link(container.xvm.keepalive);
                    canary = new Canary();
                    canary.ctx = this;
                    canary.reclaim = new Reclaim<Canary>(canary, container.xvm.refQueue, container.id, 0);
    ...
    /**
     * Object used to detect the death of <em>all</em> that reference it.
     */
    ...
    public abstract static class CleanablePhantom<V> extends PhantomReference<V> {
        ...
        public CleanablePhantom<?> link(CleanablePhantom<?> that) {
            if (that != null) {
                this.next = that.next;
                that.next = this;
            }
    ...
    public static class CohortRef extends CleanablePhantom<Collectable> {

        /**
         * The size of object we're watching
         */
        long size;
        ...
        public CohortRef(Collectable toWatch, long size, Canary canary, ReferenceQueue<? super Collectable> q) {
            super(toWatch, q);
            this.size = size;
            this.canary = canary;
            canary.reclaim.adjustSize(size);
        }
        ...
        void clean(Xvm xvm) {
            if (size > 0) {
                ++xvm.buddyReclaims;
                if (canary.reclaim.adjustSize(-size) >= 0) {
                    xvm.getContainer(canary.reclaim.cid()).release(size);
                    xvm.available += size;
                } else {
            ...
            Ctx ctx = canary.ctx;
            if (ctx != null) {
                if (ctx.lastWatch == this) {
                    ctx.lastWatch = prev;
                }
                if (ctx.firstWatch == this) {
                    ctx.firstWatch = next;
                }
            }
