...
service DigestAuthenticator
    ...
    DigestAttempt[] authenticate(RequestIn request) {
        ...
        NextAuthAttempt: for (String auth : request.header.valuesOf("Authorization")) {
            ...
            if (CaseInsensitive.stringStartsWith(auth, "Digest ")) {
                if ((   String?  realmName,
                    ...
                    Int nonce;
                    if (!(nonce := Int.parse(nonceText, 16))) {
                        attempts += Corrupt;
                        continue NextAuthAttempt;
                    }
                    ...
                    // the userId is either a name or a hash; use it as an Principal "locator"
                    String locator = userId.is(String) ? userId : userId.toString(pre="");
                    if (Principal principal := realm.findPrincipal(DigestCredential.Scheme, locator)) {
                        ...
                        NextCredential: for (Credential credential : principal.credentials) {
                            ...
                            if (credential.scheme == DigestCredential.Scheme
                                ...
                                if (responseHash == expected) {
                                    ...
                                    failure ?:= credential;
                                    // TODO GG continue; // "type is only nullable" for `failure`
                                }
                            }
                        }

                        // none of the credentials matched
                        attempts += new DigestAttempt(principal, Failed, Null, failure);

                    } else {
                        // no such user
                        attempts += new DigestAttempt(locator, Failed);
                        continue NextAuthAttempt;
                    }
                } else {
                    attempts += Corrupt;
        ...
        if (!passed) {
            ...
            attempts += new DigestAttempt(Null, attempts.empty ? NoData : Known, challenges.freeze(inPlace=True));
        }
