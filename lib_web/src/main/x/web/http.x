...
package http {
    ...
    /**
     * Assuming that the body is "streaming" and contains just one file data, extract all other
     * "Content-Disposition" elements from the body reader. This method is a very specialized copy of
     * the [parseFormData()] method above.
     *
     * When this method returns, the reader is ready to stream data beyond the first file chunk
     * returned in the [FormDataFile.contents].
     *
     * @return the [FormDataFile] containing just a single head fragment of the file
     * @return an array of [FormDataText] for all the encountered attributes
     *
     * @see the [parseFormData()] method for additional explanations
     */
    (FormDataFile, FormDataText[]) parseSingleFile(BinaryInput reader, String contentHeader) {
        assert Int boundaryOffset := contentHeader.indexOf("boundary=");

        String boundary = contentHeader.substring(boundaryOffset + 9);

        Byte[] boundaryBytes = ("--" + boundary).utf8();
        Byte[] bytes         = reader.readBytes(8Kib); // REVIEW: assume 8K limit for the data
        Int    boundarySize  = boundaryBytes.size;
        ...
        FormDataText[] textData = new Array();
        FormDataFile?  fileData = Null;
            ...
            if (Int headerEnd := part.indexOf(HEADER_BOUNDARY)) {
                ...
                if (Int nameStart := header.indexOf(CONTENT_DISPOSITION_PREFIX)) {
                    ...
                    if (Int fileNameStart := header.indexOf(FILE_NAME_PREFIX),
                        ...
                        if (fileData != Null) {
                            throw new IllegalState("The body contains more than one file");
                        }
                    ...
                    if (fileName == Null) {
                        textData += new FormDataText(name, contents.unpackUtf8());
                    } else {
                        MediaType? fileType = Null;
                        if (Int typeStart := header.indexOf(CONTENT_TYPE_PREFIX)) {
                            typeStart += CONTENT_TYPE_PREFIX.size;
                        ...
                        fileData = new FormDataFile(name, contents, fileName, fileType);
                    }
                }
            }
        }
        if (fileData == Null) {
            ...
            throw new IllegalState("The body is missing file data");
        }
        return fileData, textData;
    }
