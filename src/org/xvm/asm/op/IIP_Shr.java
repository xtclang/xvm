...
/**
 * IIP_SHR rvalue-target, rvalue-ix, rvalue2 ; T[ix] >>= T
 */
public class IIP_Shr
        extends OpIndexInPlace
    {
    /**
     * Construct an IIP_SHR op for the passed target.
     *
     * @param argTarget  the target Argument
     * @param argIndex   the index Argument
     * @param argValue   the value Argument
     */
    public IIP_Shr(Argument argTarget, Argument argIndex, Argument argValue)
        {
     ...
     */
    public IIP_Shr(DataInput in, Constant[] aconst)
            throws IOException
        {
        super(in, aconst);
        }
    ...
    protected int complete(Frame frame, ObjectHandle hTarget, JavaLong hIndex, ObjectHandle hValue)
        ...
        switch (hCurrent.getOpSupport().invokeShr(frame, hCurrent, hValue, A_STACK))
            {
            case R_NEXT:
