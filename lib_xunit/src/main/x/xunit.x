 ...
 *
 * Once we have the real XUnit then this will be responsible for discovering and executing all the
 * tests in a module.
 */
    ...
    void test(Module mod) {
        ...
        if (mod.is(TestExecutor)) {
            ...
            for (Type childType : clz.toType().childTypes.values) {
                if (childType.isA(Package), Class childClass := childType.fromClass(),
                    Object o := childClass.isSingleton()) {

                    Package pkg = o.as(Package);
                    if (pkg.isModuleImport() && pkg.is(TestExecutor)) {
                        ex = pkg;
                        break;
                    }
                }
