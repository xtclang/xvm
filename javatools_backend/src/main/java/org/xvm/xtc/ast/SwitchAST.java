...
class SwitchAST extends AST {
  ...
  static SwitchAST make( ClzBuilder X, boolean expr ) {
    ...
    return new SwitchAST(flavor,kids,expr,isa,cases,rezs, cond_defines, X);
  }
  ...
  private SwitchAST( Flavor flavor, AST[] kids, boolean expr, long isa, Const[] cases, Const[] rezs, boolean cond_defines, ClzBuilder X ) {
    super(kids);
    _flavor = flavor;
    _expr = expr;
    _isa = isa;
    ...
    case ComplexTern: // Complex multi-part arms
      ...
      for( int i=0; i<clen-1; i++ ) {
        ...
        for( int j=0; j<alen; j++ )
          arms[j] = cons[j] !=null ? XValue.val(cons[j], X) : null;
      }
      break;
    ...
    case Pattern: // This is a series of singleton matches, e.g. Strings or Enums.
      for( int i=0; i<clen; i++ )
        ...
        if( _cases[i] != null ) {
          String arm = XValue.val(_cases[i], X);
    ...
    case SimpleTern:
      ...
      for( int i=0; i<clen-1; i++ )
        _armss[i] = new String[]{XValue.val(cases[i], X)};
      break;
