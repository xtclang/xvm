...
public abstract class Const {
  ...
  public enum Format {
    ...
    /** All the Format enums in an ordinal array. */
  ...
  public enum AsnOp {
    ...
    OrAsn         ("|="  , true ),
    XorAsn        ("^="  , true ),
    AsnIfNotFalse (":="  , false),     // x := y; (includes when used as a condition, e.g. if (x := y))
    AsnIfNotNull  ("?="  , false),     // x ?= y; (includes when used as a condition, e.g. if (x := y))
    AsnIfWasTrue  ("&&=" , false),
    AsnIfWasFalse ("||=" , false),
    AsnIfWasNull  ("?:=" , false),
    Deref         ("->"  , false),
    ;
    public final String text;
    public final boolean _meth; // Has a MethodCon in the BAST
    AsnOp(String text, boolean meth) { this.text = text; _meth = meth; }
