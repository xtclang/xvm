...
class DefaultUriRouteMatch
    ...
    Function<Tuple, Tuple> createFunction()
        {
        TODO return route.executable.createFunction();
        }

    @Override
    Boolean conditionalResult.get()
        {
        TODO return route.executable.conditionalResult;
        }
    ...
    Boolean canConsume(MediaType? mediaType)
        ...
        if (route.consumes.size > 0 && mediaType.is(MediaType))
            {
            TODO return mediaType == MediaType.ALL_TYPE || explicitlyConsumes(mediaType);
            }
        else
            {
            return True;
    ...
    private Boolean anyMediaTypesMatch(MediaType[] producedTypes, MediaType[] acceptableTypes)
        ...
        TODO if (producedTypes.contains(MediaType.ALL_TYPE))
            {
            return True;
            }
        ...
        for (MediaType acceptableType : acceptableTypes)
            {
            TODO if (acceptableType == MediaType.ALL_TYPE || producedTypes.contains(acceptableType))
                {
                return True;
                }
            }
