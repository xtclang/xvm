...
import binders.BodyParameterBinder;
import binders.BindingResult;
import binders.ParameterBinder;
import binders.RequestBinderRegistry;
...
class Router
    ...
    void addRoutes(Object webService, String rootPath = "", PreProcessor[] preProcessors = [], PostProcessor[] postProcessors = [])
        ...
        for (Method<Object, Tuple, Tuple> endpoint : type.methods)
            {
            TODO if (endpoint.is(HttpEndpoint))
                {
     ...
     */
    private List<UriRouteMatch> findClosestRoute(Request req)
        {
        HttpMethod  method        = req.method;
        Boolean     permitsBody   = method.permitsRequestBody;
        MediaType?  contentType   = TODO req.contentType;
        MediaType[] acceptedTypes = TODO req.accepts;
...
TODO            if (!mostSpecific.empty || !acceptedTypes.contains(MediaType.ALL_TYPE))
                {
        ...
        if (matches.size > 1 && permitsBody)
            ...
            for (UriRouteMatch match: matches)
                {
TODO                MediaType explicitType = contentType ?: MediaType.ALL_TYPE;
     ...
     * @param req     the Request
     */
    List<UriRouteMatch> findRoutes(HttpMethod method, Request req)
        {
    ...
    (Int, String[], String[][], Byte[]) handle(String uri, String methodName, String[] headerNames,
        ...
        try
            {
            Header              headers = TODO new HttpHeaders(headerNames, headerValues);
            HttpMethod          method  = HttpMethod.fromName(methodName);
            Request             httpReq = TODO new Request(new URI(uri), headers, method, body);
...
TODO                httpReq.attributes.add(HttpAttributes.ROUTE, matchedRoute.route);
TODO                httpReq.attributes.add(HttpAttributes.ROUTE_MATCH, matchedRoute);
                ...
                if (bound.conditionalResult && result.size > 0 && result[0].as(Boolean) == False)
                    ...
                    response = Response.encodeResponse(result, index, method, mediaType);
                ...
                // ToDo: should be handled by a 404 status handler if one has been added
TODO                response = new Response(HttpStatus.NotFound);
                }
            else
                {
...
TODO                response = new Response(HttpStatus.MultipleChoices);
                }

            // ToDo: Check the status and execute any status handler for the status

            return TODO response.asTuple();
            }
